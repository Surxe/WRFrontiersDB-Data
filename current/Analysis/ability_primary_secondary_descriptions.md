# Primary and Secondary Parameters in Ability Descriptions (zh-Hans)

## 供给型装备

* 燃料储备: 以每跳{**主.x**}燃料单位点的速度补充最多{**副.x**}燃料单位点燃料。
* 燃料烧尽: 烧掉目标{**副.x**}%的燃料并阻止其自动恢复{**主.x**}秒。冷却时间：{CD}，可使用{Consum}次。
* 冲击波: 在{**副.x**}米范围内埋下一颗炸弹。炸弹会在短暂延迟后爆炸，造成{**主.x**}点伤害并击退敌人。
* 压制者: 部署一个可吸附的发射器，使范围内敌人的伤害降低{**主.x**}%，持续{**副.x**}秒。
* 快速修复: 受损最严重的模块每秒恢复{**主.x**}%的装甲，持续{**副.x**}秒。
* 动能脉冲: 发射一枚可附着于表面或机甲的射弹，释放 {**主.x**} 次脉冲，将 {**副.x**}米 半径内的敌人向四周推开。冷却时间{CD}。消耗 {Consum} 发。
* 闪现: {**副./**}秒延迟后，朝面朝方向传送{**主.x**}米距离。
* 幽灵炮塔: 部署一个炮塔，攻击距离最近的敌人，每次命中造成{**主.x**}点伤害，持续存在{**副.x**}秒。
* 冲刺反应器: 使你的最大速度提高{**副.x**}千米/小时，持续{**主.x**}秒。
* 应急护盾: 每秒恢复{**主.x**}点护盾，持续{**副.x**}秒。
* 弹药制造器: 部署一个装置，使{**副.x**}米范围内队友的装弹时间缩短{**主.x**}%。
* 能量墙: 部署一道{**主.x**}米宽的屏障，该屏障可阻挡敌人，且最多能吸收{**副.x**}点伤害。
* 反冲器: 对敌人造成{**副.x**}点伤害，并以{**主.x**}m/s的击退力将你和敌人相互推离。
* 全息场: 部署一道{**副.x**}米宽的墙阻挡视线，持续存在{**主.x**}秒。

## 躯干

* Mesa's 协调大师: Mesa使{**副.x**}米范围内队友每秒恢复{**主.x**}点护盾，自身恢复对半护盾量，并使范围内敌人的伤害输出降低30%。冷却时间: {CD}。
* Tyr's 维修无人机: 部署一架治疗无人机，在 {**副.x**}秒 内为 {**主.x**}米 范围内的友军每秒恢复 2100 装甲。冷却时间：{CD}。
* Decker's 连锁过载: 感染一名敌方机器人，对其施加病毒，在6秒内每秒造成 {**主.x**} 点伤害。效果期间，若有另一名敌人进入 {**副.x**}米 范围，病毒将传染给对方。此效果最多可连锁8次。
* Bulgasari's 铁雨: 轰炸一片区域，每枚火箭弹可对{**副.x**}米爆炸半径范围内的所有模块造成{**主.x**}点伤害。
* Cyclops's 死亡射线: 短暂延迟后，释放一道能量束，在 {**副.x**}秒 内每秒造成 {**主.x**} 点伤害。激活期间，你的机器人无法移动。
* Varangian's 能量穹顶: 部署一道半径{**副.x**}米的屏障，该屏障最多可吸收{**主.x**}点敌方攻击伤害。
* Artemis's 猎人印记: 发射一枚炮弹，击中时显示敌人，并在 {**副.x**}秒 内使其受到的护甲伤害增加 20%。如果目标在受影响期间被摧毁，所有在过去 10 秒内对其造成伤害的盟友，其每个护甲区域都将恢复 {**主.x**} 点护甲。冷却时间：{CD}。
* Relic Bulgasari's 琶音: 向目标区域发动从天而降的火箭弹齐射。每枚火箭弹的爆炸半径为 {**副.x**}米，并对爆炸区域内的每个机器人部件造成 {**主.x**} 点伤害。冷却时间：{CD}。
* Purifier's 狂徒: 附近队友造成的伤害提高{**副.x**}%，持续{**主.x**}秒。每次队友完成击杀或摧毁护盾，该效果都会得到强化。
* Loki's 全息诱饵: 隐身并投射出自己机器人的一个复制体，持续{**主.x**}秒。该诱饵拥有{**副.x**}%点额外装甲和护盾，且所有生效的效果（包括装备模块上的效果）均转移至其身上。
* Heike's 护甲蜕壳: 释放护盾脉冲，对70米范围内的敌方护盾造成 {**主.x**} 伤害，并在 {**副.x**}秒 秒内为 Heike 提供30%伤害抗性，同时为范围内的盟友提供15%伤害抗性。
* Orochi's 回声炮火: 对一条直线上的目标造成{**主.x**}点护盾伤害，并暴露敌人{**副.x**}秒秒。
* Fenrir's 诸神黄昏: 牺牲当前护甲的 {**副.Cx**}%，使伤害提升 25%，持续 10 秒。按造成伤害的 {**主.x**}% 恢复护盾和结构。冷却时间：{CD}。
* Typhon's 弹药管制: 发射一波追踪飞弹，强制最多3名敌人重新装填武器，同时使其装弹速度降低{**主.x**}%，持续{**副.x**}秒。
* Garuda's 捕蛇者: 发射一个模块，命中后展开能量牢笼，阻止敌方机器人通过。牢笼持续 {**主.x**}秒。被困其中的敌人射速和武器散布降低 {**副.x**}%。
* Wyrm's 回收者: 向目标发射一枚磁力鱼枪，并将其拉近，持续 {**主.x**}秒。激活期间，机器人对前方区域内的每个敌方机器人部件每秒造成 {**副.x**} 点伤害。冷却时间：{CD}。
* Raven's 钢之羽: 向正前方发射一波飞弹，每枚飞弹可对{**副.x**}米爆炸半径范围内的目标造成{**主.x**}点伤害。
* Phantom's 传送器: 激活后会标记一个地点，持续{**主.x**}秒，再次激活会在{**副./**}秒延迟后传送回该地点。
* Scorpion's 尾后针: 发射一枚追踪炮弹，命中时造成{**主.x**}点伤害，随后对目标所有模块造成每秒{**副.x**}点的持续伤害。如果目标在技能生效期间被摧毁，则该技能立即结束冷却。
* Harpy's 凝固汽油弹: 发射一枚炮弹，命中时造成{**副.x**}点爆炸伤害，随后造成{**主.x**}点持续火焰伤害。
* Bulwark's 缴械领域: 受到的伤害降低{**副.x**}%，持续{**主.x**}秒。球形范围内捕获到的炮弹会补充你的弹药。
* Kumo's 传送甲: 机器人传送至选定位置，并为自身及附近所有盟友施加 {**主.x**}% 的伤害减免效果。该增益持续 {**副.x**}秒。
* Anansi's 迷彩网: 制造一张半径{**副.x**}米的迷彩网，持续存在{**主.x**}秒，网外的敌人将无法看到网内的队友。冷却时间：{CD}。
* Fury's 曲速装弹: {**副.x**}%延迟后，补充{**主./**}秒的弹药。
* Pursuer's 光学伪装: 在{**副./**}秒秒延迟后进入隐身状态，持续{**主.x**}秒。机器人在隐身期间可使用装备模块，但在使用火力武器后会马上暴露。冷却时间：{CD}
* Ares's 盛怒: 激活一道内置屏障以及额外的武器，武器每次命中造成{**主.x**}点伤害，持续{**副.x**}秒。
* Griffin's 扰频器: 使敌人无法锁定{**主.x**}米范围内的队友，并将受到的伤害均摊给所有模块。持续{**副.x**}秒。所有受影响的机器人均无法被摧毁。
* Lancelot's 战斗指令: {**主.x**}米范围内的所有队友受到的伤害降低{**副.x**}%。队友被命中时，你的装甲会再生。
* Ravana's 守护者母体: 位于你前方的队友受到的伤害降低{**副.x**}%，持续{**主.x**}秒。当有任何队友受此效果影响时，你无法被摧毁。
* Siren's 寒流: 发射一枚冷凝弹，对爆炸半径内的敌人造成{**主.x**}点伤害，并使其无法移动，持续{**副.x**}秒。
* Helios's 烈焰骑手: 激活持续{**副.x**}秒，在机器人下方留下凝固汽油池。水池使敌方机器人减速30%，并对受影响区域内的每个机器人部件每秒造成{**主.x**}伤害。激活时，机器人可以随时传送回初始位置。冷却时间：{CD}。
* Crix's 钩索骇客: 释放一群纳米机器人，在 {**主.x**}秒 内锁定目标，将机器人拖向目标并造成 {**副.x**} 点伤害。
* Ceres's 回春: 每秒修复{**主.x**}%点装甲。如果{**副.x**}米内有队友，则一并修复且修复速度加倍。持续12秒。
* Relic Scorpion's 暗影突袭: 将机器人传送至多{**主.x**}米。重新出现时，对区域内的所有敌人造成25000点护盾伤害。此外还会激活内置尾炮，持续{**副.x**}秒，每秒造成3000点伤害。冷却时间：{CD}。
* Relic Lancelot's 圣骑士之护: 提供35%的伤害抗性，持续{**主.x**}秒。激活时，附近的盟友可恢复等同于Lancelot吸收伤害{**副.x**}%的护甲。冷却时间：{CD}。
* Hitcher's 带翼无人机: 生成一架翼展为{**副.x**}的无人机，对沿途所有敌人造成{**主.x**}点伤害。

## 循环型装备

* 闪光弹: 发射一枚追踪炮弹，造成{**副.x**}点伤害并致盲目标，持续{**主.x**}秒。
* 侦察模式: 提升机器人的速度，并使{**副.x**}米范围内的所有敌人暴露，持续{**主.x**}秒。
* 纳米修复: 每跳为所有模块恢复{**主.x**}%的装甲，持续{**副.x**}秒。
* 补给激增: 准备{**主./**}秒, 恢复1点充能并缩短“供给型装备”的冷却{**副.x**}%. 消耗: {ChargeDrain}.
* 反弹: 将{**副.x**}%所受的伤害返还给敌人，持续{**主.x**}秒。
* 无限弹药: 在{**副./**}秒延迟后提供无限弹药，持续{**主.x**}秒。
* 追踪飞弹: 发射一枚追踪飞弹，对爆炸区域的所有装甲部位造成{**主.x**}点伤害，或对护盾造成{**副.x**}点伤害。
* 伽马光束: 锁定敌人{**副.x**}秒，每秒造成最多{**主.x**}点伤害。距离越近，造成的伤害越高。
* 雷区: 部署一片持续存在{**副.x**}秒的隐形雷区，每颗地雷在引爆时都会使敌人减速，并对其造成{**主.x**}点伤害。
* 保护伞: 部署一道半径{**副.x**}米的屏障，该屏障可抵御来自上方的攻击，持续存在{**主.x**}秒。
* 铁幕: {**主.x**}%效果造成的所有伤害降低，持续{**副.x**}秒。需要充能：{ChargeDrain}。


# Primary and Secondary Parameters in Ability Descriptions (de)

## Zubehör

* Kraftstoffreserve: Füllt bis zu {**Sekundär.x**} Kraftstoffeinheiten auf. Regeneriert {**Primär.x**} Kraftstoffeinheiten pro Tick.
* Kraftstoff- verbrauch: Verbrennt {**Sekundär.x**}% Kraftstoff des Ziels und blockiert die Regeneration {**Primär.x**}s lang. Abklingzeit {CD}, {Consum} Aufladungen.
* Explosionswelle: Platziert eine Bombe im Umkreis von {**Sekundär.x**}m. Diese verursacht nach einer kurzen Verzögerung {**Primär.x**} Schaden und schlägt Feinde zurück.
* Entstörer: Setzt einen klebrigen Strahler ein, der {**Sekundär.x**}s lang den Schaden durch Feinde in einem Umkreis von {**Primär.x**}% verringert.
* Schnellreparatur: Repariert dein am meisten beschädigtes Modul um {**Primär.x**} Panzerung pro Sekunde. Dauert {**Sekundär.x**}s.
* Kinetischer Impuls: Feuert ein Geschoss, das an einer Oberfläche oder einem Roboter haftet, {**Primär.x**} Pulse aussendet und dabei Gegner in einem Radius von {**Sekundär.x**}m in verschiedene Richtungen wegstößt. Abklingzeit {CD}. {Consum} Ladungen.
* Blinzeln: Teleportiert dich nach einer Verzögerung von {**Sekundär./**}s {**Primär.x**}m weit in Blickrichtung.
* Ghost-Geschütz: Aktiviert ein Geschütz, das den nächsten Feind angreift und ihm im Verlauf von {**Sekundär.x**}s {**Primär.x**} Schaden pro Schuss zufügt.
* Sprintreaktor: Steigert deine maximale Geschwindigkeit {**Primär.x**}s lang um {**Sekundär.x**}km/h.
* Notfallschild: Stellt {**Sekundär.x**}s lang {**Primär.x**} Schildpunkte pro Sekunde wieder her.
* Munitionsgenerator: Aktiviert ein Gerät, das die Nachladezeit aller Verbündeter im Umkreis von {**Sekundär.x**}m um {**Primär.x**}% verringert.
* Energiewand: Aktiviert eine {**Primär.x**}m breite Barriere, die Feinde aufhält und bis zu {**Sekundär.x**} Schaden absorbiert.
* Repulsor: Fügt Feinden {**Sekundär.x**} Schaden zu und schiebt sie und dich mit einer Kraft von {**Primär.x**}m/s voneinander weg.
* Holofeld: Aktiviert eine {**Sekundär.x**}m breite Wand, die die Sichtlinie {**Primär.x**}s lang blockiert.

## Torso

* Mesa's Harmonizer: Stellt bei Verbündeten im Umkreis von {**Sekundär.x**}m {**Primär.x**} Schildpunkte pro Sekunde sowie die Hälfte dieses Werts für Mesa selbst wieder her. Reduziert außerdem den Schaden von Gegnern im Umkreis um 30%. Abklingzeit: {CD}.
* Tyr's Drohnenreparatur: Setzt eine Heilungsdrohne ein, die Verbündeten im Umkreis von {**Primär.x**}m für {**Sekundär.x**}s pro Sekunde 2100 Panzerung wiederherstellt. Abklingzeit: {CD}.
* Decker's Kettenüberlastung: Infiziert einen gegnerischen Roboter mit einem Virus, der 6 Sekunden lang {**Primär.x**} Schaden pro Sekunde verursacht. Gelangt ein anderer Gegner während der Wirkungsdauer in den {**Sekundär.x**}m-Bereich, springt der Virus auf ihn über. Der Effekt kann sich bis zu 8-mal verketten.
* Bulgasari's Eisenregen: Bombardiert ein Gebiet und fügt in einem Radius von {**Sekundär.x**}m allen Modulen {**Primär.x**} Schaden pro Rakete zu.
* Cyclops's Todesstrahl: Setzt nach einer kurzen Verzögerung einen Energiestrahl frei, der pro Sekunde {**Primär.x**} Schaden über {**Sekundär.x**}s verursacht. Dein Roboter kann während der Aktivierung nicht gehen.
* Varangian's Energiekuppel: Aktiviert eine Barriere mit einem Umkreis von {**Sekundär.x**}m, die bis zu {**Primär.x**} Schaden durch feindliche Angriffe absorbiert.
* Artemis's Mal des Jägers: Feuert ein Projektil ab, das bei Treffern einen Gegner enthüllt und den erlittenen Rüstungsschaden {**Sekundär.x**}s lang um 20 % erhöht. Wenn das Ziel zerstört wird, während es betroffen ist, stellen alle Verbündeten, die ihm in den letzten 10 Sekunden Schaden zugefügt haben, an jeder Rüstungszone {**Primär.x**} Rüstung wieder her. Abklingzeit: {CD}.
* Relic Bulgasari's Arpeggio: Feuert eine Raketensalve ab, die das Zielgebiet von oben trifft. Jede Rakete hat einen Explosionsradius von {**Sekundär.x**}m und fügt jeder Robotkomponente innerhalb des Explosionsbereichs {**Primär.x**} Schaden zu. Abklingzeit: {CD}.
* Purifier's Eiferer: Verbündete in der Nähe verursachen {**Primär.x**}s lang {**Sekundär.x**}% mehr Schaden. Der Effekt erhöht sich durch Kills oder wenn Schilde zerstört werden.
* Loki's Holografischer Köder: Lässt dich unsichtbar werden und projiziert {**Primär.x**}s lang eine Kopie von dir. Der Köder besitzt {**Sekundär.x**}% mehr Panzer- und Schildpunkte. Alle aktiven Effekte, auch die von Ausrüstungsmodulen, werden darauf übertragen.
* Heike's Panzerabwurf: Entlädt seine Schilde in einem 70-m-Impuls, verursacht {**Primär.x**} Schaden an feindlichen Schilden und gewährt Heike 30 % Schadensresistenz sowie Verbündeten im Bereich 15 % für {**Sekundär.x**}s Sekunden.
* Orochi's Echosalve: Verursacht {**Primär.x**} Schildschaden in einer geraden Linie und enthüllt Gegner {**Sekundär.x**}s lang.
* Fenrir's Ragnarok: Opfert {**Sekundär.Cx**}% der aktuellen Panzerung, um den Schaden 10 Sekunden lang um 25 % zu erhöhen. Stellt Schild und Struktur in Höhe von {**Primär.x**}% des verursachten Schadens wieder her. Abklingzeit: {CD}.
* Typhon's Blackout: Feuert Zielsuchraketen ab, die bis zu 3 Feinde dazu zwingt, nachzuladen. Gleichzeitig verlangsamt sie ihre Nachladezeiten {**Sekundär.x**}s lang um {**Primär.x**}%.
* Garuda's Schlangenfänger: Feuert ein Modul ab, das beim Aufprall einen Energiekäfig erzeugt und verhindert, dass feindliche Roboter ihn durchqueren. Der Käfig hält {**Primär.x**}s. Gefangene Gegner erleiden eine Strafe von {**Sekundär.x**}% auf Feuerrate und Streuung.
* Wyrm's Rückforderer: Feuert eine Magnetharpune auf ein Ziel ab und zieht es für {**Primär.x**}s heran. Während die Fähigkeit aktiv ist, fügt der Robot jeder gegnerischen Robotkomponente im Bereich vor ihm {**Sekundär.x**} Schaden pro Sekunde zu. Abklingzeit: {CD}.
* Raven's Stahlfedern: Entfesselt ein Raketen-Sperrfeuer direkt voraus. Jede Rakete verursacht {**Primär.x**} Schaden und hat einen Explosionsradius von {**Sekundär.x**}m.
* Phantom's Translokator: Aktiviere dies, um {**Primär.x**}s lang einen Ort zu markieren. Aktiviere dies erneut, um nach einer Verzögerung von {**Sekundär./**}s zu diesem Ort zu warpen.
* Scorpion's Peitschenschlag: Ein Zielsuchprojektil verursacht beim Aufprall {**Primär.x**} Schaden und fügt anschließend allen Modulen {**Sekundär.x**} pro Sekunde zu. Wird aufgeladen, wenn das Ziel während der Aktivität zerstört wird.
* Harpy's Napalmbombe: Feuert ein Projektil ab, das beim Aufprall {**Sekundär.x**} Explosionsschaden und anschließend {**Primär.x**} Feuerschaden über Zeit verursacht.
* Bulwark's Absorptionssphäre: Mildert {**Primär.x**}s lang {**Sekundär.x**}% des eintreffenden Schadens. Projektile, die in der Sphäre aufgefangen werden, füllen deine Munition wieder auf.
* Kumo's WarpPanzerung: Der Roboter teleportiert sich zu einem ausgewählten Ort und gewährt sich selbst sowie nahegelegenen Verbündeten eine Schadensreduktion von {**Primär.x**}%. Der Effekt hält {**Sekundär.x**}s an.
* Anansi's Tarnnetz: Erzeugt {**Primär.x**}s lang ein Tarnnetz mit einem Radius von {**Sekundär.x**}m, das Verbündete innerhalb des Netzes für Feinde außerhalb unsichtbar macht. Abklingzeit: {CD}.
* Fury's Warp-Nachladung: Füllt nach einer Verzögerung von {**Primär./**}s {**Sekundär.x**}% Munition wieder auf.
* Pursuer's Optische Tarnung: Nach einer Verzögerung von {**Sekundär./**}s Sekunden wirst du {**Primär.x**}s lang unsichtbar. Während du unsichtbar bist, kannst du Ausrüstungsmodule verwenden, aber das Abfeuern von Waffen enthüllt den Roboter sofort. Abklingzeit: {CD}.
* Ares's Zorn: Aktiviert eine integrierte Barriere und zusätzliche Waffen, die {**Sekundär.x**}s lang {**Primär.x**} Schaden pro Schuss verursachen.
* Griffin's Scrambler: Verhindert, dass Feinde Verbündete im Umkreis von {**Primär.x**}m anvisieren können und teilt den gesamten eintreffenden Schaden auf die Module auf. Dauert {**Sekundär.x**}s. Kein davon betroffene Roboter kann zerstört werden.
* Lancelot's Schlachtordnung: Verbündete im Umkreis von {**Primär.x**}m erleiden {**Sekundär.x**}% weniger Schaden. Sobald sie getroffen werden, regenerierst du Panzerung.
* Ravana's Wächtermatrix: Verbündete vor dir erleiden {**Primär.x**}s lang {**Sekundär.x**}% weniger Schaden. Du kannst nicht getötet werden, solange ein Verbündeter davon betroffen ist.
* Siren's Kälteeinbruch: Feuert ein Gefrierprojektil ab, das Feinden im Wirkradius {**Primär.x**} Schaden zufügt und sie {**Sekundär.x**}s lang bewegungsunfähig macht.
* Helios's Flammenreiter: Aktiviert für {**Sekundär.x**}s und hinterlässt Napalmlachen unter dem Roboter. Lachen verlangsamen gegnerische Roboter um 30 % und verursachen {**Primär.x**} Schaden pro Sekunde an jeder Roboterkomponente im gewählten Bereich. Solange aktiv, kann sich der Roboter jederzeit an seine Startposition zurückteleportieren. Abklingzeit: {CD}.
* Crix's Enterhaken: Setzt einen Schwarm Nanobots frei, die sich über {**Primär.x**}s an das Ziel klammern, den Roboter zu ihm ziehen und {**Sekundär.x**} Schaden verursachen.
* Ceres's Verjünger: Repariert {**Primär.x**}% Panzerung pro Sekunde. Wenn sich Verbündete im Umkreis von {**Sekundär.x**}m befinden, erhalten diese ebenfalls Reparaturen und die Rate verdoppelt sich. Hält 12 Sekunden lang an.
* Relic Scorpion's Schattenangriff: Teleportiert den Roboter bis zu {**Primär.x**}m weit. Bei der Wiedererscheinung fügt er allen Gegnern im Bereich 25000 Schildschaden zu. Aktiviert außerdem für {**Sekundär.x**}s eine eingebaute Heckkanone, die 3000 Schaden pro Sekunde verursacht. Abklingzeit: {CD}.
* Relic Lancelot's Paladins Schutz: Gewährt 35 % Schadensresistenz für {**Primär.x**}s. Während sie aktiv ist, stellen nahegelegene Verbündete Rüstung in Höhe von {**Sekundär.x**}% des von Lancelot absorbierten Schadens wieder her. Abklingzeit: {CD}.
* Hitcher's Geflügelte Drohne: Spawnt eine Drohne mit flachen Flügeln, die {**Sekundär.x**} lang sind. Diese Drohne fügt allen Feinden in ihrem Weg {**Primär.x**} Schaden zu.

## Zyklus-Ausrüstung

* Blendgranate: Feuert ein Zielsuchprojektil ab, das {**Sekundär.x**} Schaden verursacht und das Ziel {**Primär.x**}s lang blendet.
* Aufklärungsmodus: Der Roboter erhält einen Geschwindigkeitsschub und deckt alle Gegner {**Primär.x**}s lang im Umkreis von {**Sekundär.x**}m auf.
* Nanoreparatur: Repariert alle Module um {**Primär.x**}% Panzerung pro Tick. Hält {**Sekundär.x**}s lang an.
* Nachschubwelle: Stellt nach einer {**Primär./**}s Aufladezeit 1 Ladung wieder her und verkürzt die Abklingzeit des Zubehörs um {**Sekundär.x**}%. Benötigte Ladung: {ChargeDrain}.
* Reflektor: Wirft {**Primär.x**}s lang {**Sekundär.x**}% des eintreffenden Schadens auf den Feind zurück.
* Unendliche Munition: Gewährt nach einer Verzögerung von {**Sekundär./**}s {**Primär.x**}s lang unbegrenzt viel Munition.
* Zielsuchrakete: Feuert eine Zielsuchrakete ab, die allen Panzersegmenten im Explosionsbereich {**Primär.x**} Schaden zufügt oder {**Sekundär.x**} Schildschaden verursacht.
* Gammastrahl: Bleibt am Feind {**Sekundär.x**}s lang haften und fügt diesem bis zu {**Primär.x**} Schaden pro Sekunde zu. Der Schaden hängt von der Entfernung ab.
* Minenfeld: Legt ein unsichtbares Minenfeld aus, das {**Sekundär.x**}s lang bestehen bleibt. Jede Mine verlangsamt beim Aufprall Feinde und fügt ihnen {**Primär.x**} Schaden zu.
* Schirm: Aktiviert eine Barriere mit einem Radius von {**Sekundär.x**}m, die gegen Angriffe von oben schützt. Die Barriere hält {**Primär.x**}s lang an.
* Eiserner Schleier: Verringert {**Sekundär.x**}s lang sämtlichen erlittenen Schaden um {**Primär.x**}%. Erforderliche Ladung: {ChargeDrain}.


# Primary and Secondary Parameters in Ability Descriptions (pl)

## Osprzęt zasobów

* Rezerwa paliwa: Uzupełnia paliwo do maks. {**Drugoplanowy.x**} jedn. paliwa z szybkością {**Podstawowy.x**} jedn. paliwa na jedn. czasu.
* Spalanie paliwa: Spala {**Drugoplanowy.x**}% paliwa celu i blokuje przez {**Podstawowy.x**} sek jego odzyskiwanie. Czas reaktywacji: {CD}; ładunki: {Consum}.
* Fala uderzeniowa: Wystrzeliwuje bombę na odległość {**Drugoplanowy.x**} m. Eksplozja po krótkim opóźnieniu powoduje {**Podstawowy.x**} obrażeń i odrzuca roboty w polu rażenia.
* Tłumik: Powoduje wypuszczenie emitera, który przykleja się do powierzchni i zmniejsza obrażenia zadawane przez wrogów w obszarze o {**Podstawowy.x**}%; czas aktywności: {**Drugoplanowy.x**} sek.
* Szybka naprawa: Naprawia twój najmocniej uszkodzony moduł z szybkością {**Podstawowy.x**}% na sek. Czas aktywności: {**Drugoplanowy.x**} sek.
* Impuls Kinetyczny: Wystrzeliwuje pocisk, który przyczepia się do powierzchni lub robota i emituje {**Podstawowy.x**} impulsów, które odpychają przeciwników w promieniu {**Drugoplanowy.x**} m na wszystkie strony. Czas reaktywacji {CD}. {Consum} ładunków.
* Błysk: Po opóźnieniu wynoszącym {**Drugoplanowy./**} sek, teleportuje cię na odległość {**Podstawowy.x**} m w kierunku, w którym zwrócony jest robot.
* Fantomowa wieżyczka: Uaktywnia wieżyczkę, która ostrzeliwuje najbliższego wroga, powodując {**Podstawowy.x**} obrażeń na strzał. Czas aktywności: {**Drugoplanowy.x**} sek.
* Reaktor dopalacza: Zwiększa twoją maksymalną szybkość o {**Drugoplanowy.x**} km/h przez {**Podstawowy.x**} sek.
* Tarcza awaryjna: Przywraca {**Podstawowy.x**} wytrzymałości tarczy w ciągu {**Drugoplanowy.x**} sek.
* Wytwarzanie amunicji: Uaktywnia urządzenie, które zwiększa szybkość przeładowania sojuszników w promieniu {**Drugoplanowy.x**} m o {**Podstawowy.x**}%.
* Ściana energetyczna: Uaktywnia barierę osłony o szerokości {**Podstawowy.x**} m, która powstrzymuje wrogów i absorbuje do {**Drugoplanowy.x**} obrażeń.
* Odpychacz: Zadaje wrogom {**Drugoplanowy.x**} obrażeń, a przy tym odrzuca ciebie i twoich przeciwników od siebie z siłą {**Podstawowy.x**}m/s.
* Holopole: Uaktywnia mur osłonowy o szerokości {**Drugoplanowy.x**} m, który ogranicza widok przez {**Podstawowy.x**} sek.

## Korpus

* Mesa's Harmonizator: Przywraca {**Podstawowy.x**} tarcz(y) sojuszników w promieniu {**Drugoplanowy.x**} m i połowę tej wartości samej Mesie. Ponadto ogranicza o 30% obrażenia zadawane przez wrogów. Schłodzenie: {CD}.
* Tyr's Dron naprawczy: Rozmieszcza drona naprawczego, który przywraca 2100 pancerza na sekundę sojusznikom w promieniu {**Podstawowy.x**} m przez {**Drugoplanowy.x**} sek. Czas odnowienia: {CD}.
* Decker's Przeciążenie Kaskadowe: Infekuje wrogiego robota wirusem, który zadaje {**Podstawowy.x**} obrażeń na sekundę przez 6 sekund. Jeśli w tym czasie inny przeciwnik znajdzie się w zasięgu {**Drugoplanowy.x**} m, wirus przeskakuje również na niego. Może przeskoczyć do 8 razy.
* Bulgasari's Żelazny deszcz: Rozpoczyna ostrzał obszarowy, podczas którego każda rakieta zadaje {**Podstawowy.x**} obrażeń wszystkich modułów w polu rażenia {**Drugoplanowy.x**} m.
* Cyclops's Promień śmierci: Po krótkim opóźnieniu uwalnia wiązkę energii, która zadaje {**Podstawowy.x**} obrażeń na sekundę przez {**Drugoplanowy.x**} sek. Twój robot nie może chodzić, gdy umiejętność jest aktywna.
* Varangian's Kopuła energetyczna: Uaktywnia barierę osłony o promieniu {**Drugoplanowy.x**} m, która pochłania do {**Podstawowy.x**} obrażeń zadawanych przez atakujących wrogów.
* Artemis's Znak łowcy: Wystrzeliwuje pocisk, który przy trafieniu ujawnia wroga, zwiększając otrzymywane obrażenia pancerza o 20% przez {**Drugoplanowy.x**} sek. Jeśli cel zostanie zniszczony pod wpływem efektu, wszyscy sojusznicy, którzy zadali mu obrażenia w ciągu ostatnich 10 sekund, odnawiają {**Podstawowy.x**} pancerza w każdej strefie pancerza. Czas odnowienia: {CD}.
* Relic Bulgasari's Arpeggio: Wystrzeliwuje salwę rakiet, które uderzają w wyznaczony obszar z góry. Każda rakieta ma promień wybuchu wynoszący {**Drugoplanowy.x**} m i zadaje {**Podstawowy.x**} obrażeń każdemu komponentowi robota w strefie eksplozji. Czas odnowienia: {CD}.
* Purifier's Fanatyk: Pobliscy sojusznicy zadają {**Drugoplanowy.x**}% więcej obrażeń przez {**Podstawowy.x**} sek. Efekt zwiększa się po wyeliminowaniu przez sojuszników wroga lub zniszczeniu jego tarczy.
* Loki's Holograficzna przynęta: Twój robot staje się niewidzialny i wyświetla przez {**Podstawowy.x**} sek kopię holograficzną twojej maszyny; taki klon ma pancerz i tarczę większą o {**Drugoplanowy.x**}%, zostają na niego przeniesione wszystkie aktywne efekty włącznie z tymi, które mają źródło w modułach sprzętu.
* Heike's Zrzut pancerza: Rozładowuje tarcze impulsem o zasięgu 70 m, zadając {**Podstawowy.x**} obrażeń wrogim tarczom oraz zapewniając Heike 30% odporności na obrażenia, a sojusznikom w obszarze 15% na {**Drugoplanowy.x**} sek sekund.
* Orochi's Echo-ostrzał: Powoduje {**Podstawowy.x**} obrażeń tarczy w linii prostej, ujawniając przy tym na {**Drugoplanowy.x**} sek pozycje wrogów.
* Fenrir's Ragnarok: Poświęca {**Drugoplanowy.Cx**}% aktualnego pancerza, zwiększając obrażenia o 25% na 10 sekund. Przywraca osłonę i strukturę w wysokości {**Podstawowy.x**}% zadanych obrażeń. Czas odnowienia: {CD}.
* Typhon's Zaciemnienie: Wystrzeliwuje rakiety samonaprowadzające, które zmuszają maks. 3 wrogów do przeładowania broni, przy czym przeładowanie jest spowalniane o {**Podstawowy.x**}% przez {**Drugoplanowy.x**} sek.
* Garuda's Łowca węży: Wystrzeliwuje moduł, który przy trafieniu tworzy energetyczną klatkę, uniemożliwiając wrogim robotom przejście. Klatka utrzymuje się przez {**Podstawowy.x**} sek. Uwięzieni w niej wrogowie otrzymują karę {**Drugoplanowy.x**}% do szybkostrzelności i rozrzutu broni.
* Wyrm's Odzyskiwacz: Wystrzeliwuje harpun magnetyczny w stronę celu i przyciąga go przez {**Podstawowy.x**} sek. Gdy zdolność jest aktywna, robot zadaje {**Drugoplanowy.x**} obrażeń na sekundę każdemu komponentowi wrogiego robota na obszarze przed nim. Czas odnowienia: {CD}.
* Raven's Stalowe pióra: Wystrzeliwuje serię pocisków na wprost. Każdy z nich powoduje {**Podstawowy.x**} obrażeń od eksplozji w promieniu {**Drugoplanowy.x**} m.
* Phantom's Translokator: Aktywuj, żeby oznaczyć miejsce znacznika aktywnego przez {**Podstawowy.x**} sek. Ponowna aktywacja powoduje teleportowanie do wybranego miejsca po opóźnieniu {**Drugoplanowy./**} sek.
* Scorpion's Bicz: Pocisk samonaprowadzający powoduje po trafieniu {**Podstawowy.x**} obrażeń, a następnie dodatkowo {**Drugoplanowy.x**} obrażeń na sek. od ognia wszystkich modułów. Zniszczenie wroga w czasie aktywności powoduje od razu przeładowanie.
* Harpy's Pocisk z napalmem: Wystrzeliwuje rakietę, która powoduje {**Drugoplanowy.x**} obrażeń od eksplozji, a następnie (z czasem) również {**Podstawowy.x**} obrażeń od ognia.
* Bulwark's Sfera łapacza: Blokuje {**Drugoplanowy.x**}% otrzymywanych obrażeń w ciągu {**Podstawowy.x**} sek. Pociski przejęte w tym czasie uzupełniają zapasy twojej amunicji.
* Kumo's Zbroja warp: Robot teleportuje się do wybranego miejsca i nakłada na siebie oraz pobliskich sojuszników redukcję obrażeń o {**Podstawowy.x**}%. Efekt trwa {**Drugoplanowy.x**} sek.
* Anansi's Siatka kamuflażu: Tworzy Siatkę kamuflażu o promieniu {**Drugoplanowy.x**} m, która jest aktywna przez {**Podstawowy.x**} sek. Ukryci za nią sojusznicy są niewidoczni dla wrogów na zewnątrz. Czas reaktywacji {CD}.
* Fury's Przeładowanie warp: Uzupełnia {**Drugoplanowy.x**}% amunicji po opóźnieniu wynoszącym {**Podstawowy./**} sek.
* Pursuer's Kamuflaż optyczny: Po opóźnieniu trwającym {**Drugoplanowy./**} sek twój robot zyskuje na {**Podstawowy.x**} sek tymczasową niewidzialność. W tym czasie można używać modułów sprzętu, ale otwarcie ognia ujawni twoją pozycję. Czas reaktywacji: {CD}.
* Ares's Gniew: Aktywuje wbudowaną barierę i dodatkową broń, która powoduje {**Podstawowy.x**} obrażeń na strzał przez {**Drugoplanowy.x**} sek.
* Griffin's Szyfrator: Zapobiega blokadzie pocisków naprowadzanych wroga na sojusznikach w odległości {**Podstawowy.x**} m oraz rozdziela otrzymywane obrażenia pomiędzy moduły. Czas aktywności: {**Drugoplanowy.x**} sek. Robot pod wpływem nie może zostać zniszczony.
* Lancelot's Porządek bojowy: Po aktywacji sojusznicy w promieniu {**Podstawowy.x**} m otrzymują {**Drugoplanowy.x**}% mniej obrażeń. Jeśli ktoś zostanie trafiony, powoduje to regenerację twojego pancerza.
* Ravana's Strażnik: Sojusznicy znajdujący się przed tobą otrzymują {**Drugoplanowy.x**}% mniej obrażeń przez {**Podstawowy.x**} sek. W czasie aktywności tej funkcji twój robot nie może zostać zniszczony.
* Siren's Uderzenie mrozu: Wystrzeliwuje pocisk zamrażający, który powoduje {**Podstawowy.x**} obrażeń w polu rażenia i unieruchamia wrogów na {**Drugoplanowy.x**} sek.
* Helios's Płonący jeździec: Aktywuje się na {**Drugoplanowy.x**} sek, pozostawiając kałuże napalmu pod robotem. Kałuże spowalniają wrogie roboty o 30% i zadają {**Podstawowy.x**} obrażeń na sekundę każdemu komponentowi robota w obszarze działania. Gdy jest aktywna, robot może w dowolnym momencie teleportować się z powrotem do pozycji początkowej. Czas odnowienia: {CD}.
* Crix's Kotwiczenie: Wypuszcza rój nanobotów, które przywierają do celu przez {**Podstawowy.x**} sek, przyciągając do niego robota i zadając {**Drugoplanowy.x**} obrażeń.
* Ceres's Źródło regeneracji: Przywraca {**Podstawowy.x**}% pancerza na sek. Jeśli w promieniu {**Drugoplanowy.x**} m znajdują się sojusznicy, regeneracja obejmie również te jednostki (tempo napraw zostaje podwojone). Czas aktywności: 12 sek.
* Relic Scorpion's Cienisty cios: Teleportuje robota na odległość do {**Podstawowy.x**} m. Po ponownym pojawieniu się zadaje 25000 obrażeń tarczy wszystkim wrogom w obszarze. Aktywuje również wbudowane działo ogonowe na {**Drugoplanowy.x**} sek, zadając 3000 obrażeń na sekundę. Czas odnowienia: {CD}.
* Relic Lancelot's Straż Paladyna: Zapewnia 35% odporności na obrażenia przez {**Podstawowy.x**} sek. Gdy jest aktywna, pobliscy sojusznicy przywracają pancerz równy {**Drugoplanowy.x**}% obrażeń pochłoniętych przez Lancelot. Czas odnowienia: {CD}.
* Hitcher's Skrzydlaty dron: Wystrzeliwuje drona z płaskimi skrzydłami o długości {**Drugoplanowy.x**}, który zadaje {**Podstawowy.x**} obrażeń wszystkim wrogom na swojej drodze.

## Osprzęt ładowany

* Błyskawica: Wystrzeliwuje samonaprowadzający pocisk, który powoduje {**Drugoplanowy.x**} obrażeń i oślepia cel na {**Podstawowy.x**} sek.
* Tryb zwiadowcy: Robot zyskuje premię do szybkości i odkrywa pozycje wszystkich wrogów odległości {**Drugoplanowy.x**} m. Czas aktywności: {**Podstawowy.x**} sek.
* Naprawcze nanity: Naprawia wszystkie moduły z szybkością {**Podstawowy.x**}% pancerza na jedn. czasu. Czas aktywności: {**Drugoplanowy.x**} sek.
* Fala Zapasów: Po {**Podstawowy./**} sek ładowania odnawia 1 ładunek i zmniejsza czas reaktywacji Osprzętu zasobów o {**Drugoplanowy.x**}%. Potrzebny ładunek: {ChargeDrain}.
* Reflektor: Przez {**Podstawowy.x**} sek {**Drugoplanowy.x**}% otrzymanych obrażeń zostaje przekierowana z powrotem na wroga.
* Nieskończona amunicja: Po opóźnieniu wynoszącym {**Drugoplanowy./**} sek przez {**Podstawowy.x**} sek zapewnia nieograniczoną amunicję.
* Pocisk samonaprowadzający: Wystrzeliwuje rakietę samonaprowadzającą, która powoduje {**Podstawowy.x**} obrażeń wszystkim segmentom pancerza w polu rażenia po eksplozji lub {**Drugoplanowy.x**} obrażeń tarcz.
* Promień gamma: Uaktywnia urządzenie, które koncentruje się na wrogu i przez {**Drugoplanowy.x**} sek zadaje mu do {**Podstawowy.x**} obrażeń na sek. Obrażenia zmieniają się wraz z odległością od celu.
* Pole minowe: Rozmieszcza niewidoczne dla wroga pole minowe aktywne przez {**Drugoplanowy.x**} sek. Każda mina spowalnia wrogów i powoduje {**Podstawowy.x**} obrażeń.
* Parasol: Uaktywnia barierę o promieniu {**Drugoplanowy.x**} m, która chroni cię przed atakami z góry. Czas aktywności: {**Podstawowy.x**} sek.
* Żelazny welon: Redukuje wszystkie obrażenia otrzymywane przez {**Podstawowy.x**}% na {**Drugoplanowy.x**} sek. Potrzebny ładunek: {ChargeDrain}.


# Primary and Secondary Parameters in Ability Descriptions (es)

## Componente de suministro

* Reserva de combustible: Rellena hasta {**Secundario.x**} unidades de combustible de combustible a {**Primario.x**} unidades de combustible unidades por tic.
* Quema de combustible: Quema {**Secundario.x**}% combustible del objetivo y bloquea su regeneración durante {**Primario.x**} s. Tiempo de recarga {CD}, {Consum} cargas.
* Onda expansiva: Coloca una bomba en un radio de {**Secundario.x**} m. Después de un breve retardo, la explosión inflige {**Primario.x**} de daño y repele a los enemigos.
* Supresor: Despliega un emisor adhesivo que reduce el daño de los enemigos en el área en {**Primario.x**}% durante {**Secundario.x**} s.
* Reparación rápida: Repara tu módulo más dañado con un {**Primario.x**} % de armadura por segundo. Dura {**Secundario.x**} s.
* Impulso Cinético: Dispara un proyectil que se adhiere a una superficie o a un robot y emite {**Primario.x**} impulsos que empujan a los enemigos en un radio de {**Secundario.x**} m en distintas direcciones. Tiempo de recarga {CD}. {Consum} cargas.
* Parpadeo: Después de un retardo de {**Secundario./**} s, te teletransporta {**Primario.x**} m de frente.
* Torreta fantasma: Despliega una torreta que ataca al enemigo más cercano infligiendo {**Primario.x**} de daño por tiro durante {**Secundario.x**} s.
* Reactor de velocidad: Aumenta tu velocidad máxima en {**Secundario.x**} km/h durante {**Primario.x**} s.
* Escudo de emergencia: Restaura {**Primario.x**} de escudo por segundo durante {**Secundario.x**} s.
* Fabricante de munición: Despliega un dispositivo que reduce el tiempo de recarga de los aliados en un radio de {**Secundario.x**} m en {**Primario.x**}%.
* Muro energético: Despliega una barrera de {**Primario.x**} m de ancho que detiene a los enemigos y absorbe hasta {**Secundario.x**} de daño.
* Repulsor: Inflige {**Secundario.x**} de daño a los enemigos y os aparta a todos con {**Primario.x**}m/s de fuerza.
* Holocampo: Despliega un holocampo de {**Secundario.x**} m de ancho que bloquea la línea de visión durante {**Primario.x**} s.

## Torso

* Mesa's Armonizador: Restaura {**Primario.x**} de escudo por segundo para los aliados en un radio de {**Secundario.x**} m y la mitad del valor para Mesa. También reduce un 30% el daño infligido por los enemigos. Tiempo de recarga: {CD}.
* Tyr's Dron de reparación: Despliega un dron de curación que restaura 2100 de armadura por segundo a los aliados dentro de {**Primario.x**} m durante {**Secundario.x**} s. Enfriamiento: {CD}.
* Cubierta's Sobrecarga en Cadena: Infecta a un robot enemigo con un virus que inflige {**Primario.x**} de daño por segundo durante 6 segundos. Si otro enemigo entra en un radio de {**Secundario.x**} m durante el efecto, el virus salta a él. Puede rebotar hasta 8 veces.
* Bulgasari's Lluvia de hierro: Bombardea un área infligiendo {**Primario.x**} de daño por cohete a todos los módulos en un radio de explosión de {**Secundario.x**} m.
* Cyclops's Rayo de la muerte: Tras un breve retraso, lanza un haz de energía que inflige {**Primario.x**} de daño por segundo durante {**Secundario.x**} s. Tu robot no puede caminar mientras esté activa.
* Varangian's Cúpula energética: Despliega una barrera con un radio de {**Secundario.x**} m que absorbe hasta {**Primario.x**} de daño de los ataques enemigos.
* Artemis's Marca del cazador: Dispara un proyectil que revela a un enemigo al impactar, lo que aumenta el daño de armadura recibido en un 20 % durante {**Secundario.x**} s. Si el objetivo es destruido mientras está afectado, todos los aliados que le hayan infligido daño en los últimos 10 segundos restauran {**Primario.x**} de armadura en cada zona de armadura. Enfriamiento: {CD}.
* Relic Bulgasari's Arpeggio: Lanza una salva de cohetes que impacta el área objetivo desde arriba. Cada cohete tiene un radio de explosión de {**Secundario.x**} m e inflige {**Primario.x**} de daño a cada componente de robot dentro del área de la explosión. Tiempo de recarga: {CD}.
* Purifier's Fanático: Los aliados cercanos infligen {**Secundario.x**}% más de daño durante {**Primario.x**} s. El efecto aumenta cuando se causan bajas o se destruyen escudos.
* Loki's Señuelo holográfico: Te hace invisible y proyecta tu copia durante {**Primario.x**} s. El señuelo tiene {**Secundario.x**}% más armadura y escudos, y todos los efectos activos, incluidos los de los módulos de equipo, se transfieren a él.
* Heike's Muda defensiva: Descarga sus escudos en un pulso de 70 m que inflige {**Primario.x**} a los escudos enemigos y otorga 30 % de resistencia al daño a Heike y 15 % a los aliados dentro del área durante {**Secundario.x**} s segundos.
* Orochi's Aluvión de ecos: Inflige {**Primario.x**} de daño de escudo en línea recta revelando enemigos durante {**Secundario.x**} s.
* Fenrir's Ragnarok: Sacrifica {**Secundario.Cx**}% de la armadura actual para aumentar el daño un 25 % durante 10 segundos. Restaura escudo y estructura en {**Primario.x**}% del daño infligido. Enfriamiento: {CD}.
* Typhon's Apagón: Dispara misiles teledirigidos que obligan hasta a 3 enemigos a recargar mientras se ralentizan sus recargas {**Primario.x**}% durante {**Secundario.x**} s.
* Garuda's Cazador de serpientes: Dispara un módulo que despliega una jaula de energía al impactar, impidiendo que los robots enemigos la atraviesen. La jaula dura {**Primario.x**} s. Los enemigos atrapados sufren una penalización de {**Secundario.x**}% a la cadencia de fuego y la dispersión de las armas.
* Wyrm's Reclamador: Dispara un arpón magnético a un objetivo y lo arrastra hacia ti durante {**Primario.x**} s. Mientras esté activa, el robot inflige {**Secundario.x**} de daño por segundo a cada componente de robot enemigo en el área frente a él. Tiempo de recarga: {CD}.
* Raven's Plumas de acero: Lanza una descarga de misiles hacia delante. Cada uno inflige {**Primario.x**} de daño con un radio de explosión de {**Secundario.x**} m.
* Phantom's Translocalizador: Actívalo para marcar un punto durante {**Primario.x**} s. Actívalo de nuevo para volver a teletransportarte hasta él con un retardo de {**Secundario./**} s.
* Scorpion's Latigazo: Un proyectil teledirigido inflige {**Primario.x**} de daño al impactar y {**Secundario.x**} por segundo a todos los módulos. Se recarga si el objetivo es destruido mientras está activo.
* Harpy's Caparazón de napalm: Dispara un proyectil que inflige {**Secundario.x**} de daño de explosión al impactar y {**Primario.x**} de daño de fuego prolongado.
* Bulwark's Esfera absorbente: Mitiga {**Secundario.x**}% del daño entrante durante {**Primario.x**} s. Los proyectiles atrapados en la esfera recargan tu munición.
* Kumo's Armadura de teletransporte: El robot se teletransporta a una posición seleccionada y otorga una reducción de daño de {**Primario.x**}% a sí mismo y a los aliados cercanos. El efecto dura {**Secundario.x**} s.
* Anansi's Red de camuflaje: Crea una Red de camuflaje con un radio de {**Secundario.x**} m durante {**Primario.x**} s, que hace a los aliados que están dentro invisibles a los enemigos de fuera. Tiempo de recarga: {CD}.
* Fury's Recarga de distorsión: Rellena {**Secundario.x**}% de munición tras un retardo de {**Primario./**} s.
* Pursuer's Camuflaje óptico: Tras un retardo de {**Secundario./**} s, te hace invisible durante {**Primario.x**} s. Los módulos de los componentes pueden usarse mientras son invisibles, pero disparar armas revela el robot de inmediato. Tiempo de recarga: {CD}.
* Ares's Furia: Activa una barrera integrada y armas extra que infligen {**Primario.x**} de daño por disparo durante {**Secundario.x**} s.
* Griffin's Codificador: Evita que los enemigos apunten a los aliados en un radio de {**Primario.x**} m y divide el daño entrante entre los módulos. Dura {**Secundario.x**} s. Cualquier robot afectado no puede ser destruido.
* Lancelot's Orden de combate: Los aliados en un radio de {**Primario.x**} m reciben {**Secundario.x**}% menos de daño. Cuando son alcanzados, tú regeneras armadura.
* Ravana's Matriz de guardianes: Los aliados frente a ti reciben {**Secundario.x**}% menos de daño durante {**Primario.x**} s. No te podrán matar mientras haya aliados con este efecto activo.
* Siren's Ola de frío: Dispara un proyectil congelador que inflige {**Primario.x**} de daño a los enemigos en un radio de explosión y los inmoviliza durante {**Secundario.x**} s.
* Helios's Jinete flameante: Se activa durante {**Secundario.x**} s, dejando charcos de napalm bajo el robot. Los charcos ralentizan a los robots enemigos un 30% e infligen {**Primario.x**} de daño por segundo a cada componente de robot en el área afectada. Mientras está activa, el robot puede teletransportarse de vuelta a su posición inicial en cualquier momento. Tiempo de recarga: {CD}.
* Crix's Agarre: Lanza un enjambre de nanobots que se enganchan al objetivo durante {**Primario.x**} s, arrastrando al robot hacia él e infligiendo {**Secundario.x**} de daño.
* Ceres's Regenerador: Repara un {**Primario.x**} % de armadura por segundo. Si hay algún aliado en un radio de {**Secundario.x**} m, también lo reparará y duplicará la velocidad de reparación. Dura 12 segundos.
* Relic Scorpion's Ataque sombrío: Teletransporta al robot hasta {**Primario.x**} m. Al reaparecer, inflige 25000 de daño de escudo a todos los enemigos en el área. También activa un cañón de cola integrado durante {**Secundario.x**} s, que inflige 3000 de daño por segundo. Tiempo de recarga: {CD}.
* Relic Lancelot's Protección del paladín: Otorga un 35% de resistencia al daño durante {**Primario.x**} s. Mientras está activa, los aliados cercanos restauran armadura equivalente al {**Secundario.x**}% del daño absorbido por Lancelot. Tiempo de recarga: {CD}.
* Hitcher's Dron alado: Genera un dron con alas planas de {**Secundario.x**} de largo que inflige {**Primario.x**} de daño a todos los enemigos que encuentra a su paso.

## Componente cíclico

* Explosión cegadora: Dispara un proyectil teledirigido que inflige {**Secundario.x**} de daño y ciega al objetivo durante {**Primario.x**} s.
* Modo de reconocimiento: El robot obtiene un potenciador de velocidad y revela a todos los enemigos en un radio de {**Secundario.x**} m durante {**Primario.x**} s.
* Reparación de nanobots: Repara un {**Primario.x**} % de armadura por tic en todos los módulos. Dura {**Secundario.x**} s.
* Pico de Suministro: Tras {**Primario./**} s de preparación, recupera 1 carga y reduce el tiempo de recarga del Componente de suministro en {**Secundario.x**}%. Coste de carga: {ChargeDrain}.
* Reflector: Devuelve {**Secundario.x**}% del daño entrante al enemigo durante {**Primario.x**} s.
* Munición infinita: Después de un retardo de {**Secundario./**} s, proporciona munición ilimitada durante {**Primario.x**} s.
* Misil teledirigido: Lanza un misil teledirigido que inflige {**Primario.x**} de daño a todos los segmentos de armadura del radio de explosión o a {**Secundario.x**} escudos.
* Rayo gamma: Sigue al enemigo durante {**Secundario.x**} s e inflige hasta {**Primario.x**} de daño por segundo. El daño aumenta con la proximidad.
* Campo de minas: Despliega un campo de minas invisible que dura {**Secundario.x**} s. Cada mina ralentiza a los enemigos al impactar e inflige {**Primario.x**} de daño.
* Paraguas: Despliega una barrera con un radio de {**Secundario.x**} m que protege contra los ataques desde arriba. Dura {**Primario.x**} s.
* Velo de hierro: Reduce todo el daño recibido en {**Primario.x**}% durante {**Secundario.x**} s. Carga necesaria: {ChargeDrain}.


# Primary and Secondary Parameters in Ability Descriptions (ko)

## 보급 기어

* 연료 비축: 틱당 {**주.x**}연료씩 최대 {**보조.x**}연료까지 연료를 보충합니다.
* 연료 연소: 대상에서 {**보조.x**}% 연료를 태워서 {**주.x**}초 동안 재생을 차단해. 쿨타임 {CD}, {Consum} 충전이야.
* 폭발 파동: {**보조.x**}m 내에 폭탄을 설치합니다. 짧은 지연 후 폭발하여 {**주.x**} 피해를 입히고 적을 밀쳐냅니다.
* 억제기: 적에게 주는 피해를 {**주.x**}%만큼 줄여주는 스틱 발사기를 배치해. 지속 시간은 {**보조.x**}초야.
* 급속 수리: 가장 손상된 모듈의 아머를 초당 {**주.x**}% 수리합니다. {**보조.x**}초 동안 지속됩니다.
* 운동 충격파: 표면이나 로봇에 달라붙는 발사체를 발사해 {**주.x**}회의 충격파를 방출하며, {**보조.x**}m 반경 내의 적을 사방으로 밀어냅니다. 쿨다운 {CD}. {Consum}회분.
* 점멸: {**보조./**}초 지연 후, 바라보는 방향으로 {**주.x**}m 순간이동합니다.
* 유령 포탑: 가장 가까운 적을 공격하는 포탑을 배치하여 {**보조.x**}초 동안 한 발당 {**주.x**} 피해를 입힙니다.
* 질주 반응로: {**주.x**}초 동안 최대 속도를 {**보조.x**}km/h 증가시킵니다.
* 긴급 실드: {**보조.x**}초 동안 초당 실드가 {**주.x**} 재생됩니다.
* 탄약 제조기: {**주.x**}% 내 아군의 재장전 시간을 {**주.x**}% 감소시키는 장치를 배치합니다.
* 에너지 벽: 적을 막고 최대 {**보조.x**} 피해를 흡수하는 {**주.x**}m 너비의 장벽을 배치합니다.
* 반발기: 적에게 {**보조.x**}의 피해를 주고 {**주.x**}m/s의 힘으로 플레이어와 적이 서로를 밀어냅니다.
* 홀로필드: {**주.x**}초 동안 시야를 차단하는 {**보조.x**}m 너비의 벽을 배치합니다.

## 몸통

* 메사's 조율기: {**보조.x**}m 내에 있는 아군의 실드를 초당 {**주.x**} 회복시키고, 메사 자신은 그 절반만큼 회복합니다. 또한 사거리 내의 적이 가하는 피해량이 30% 감소합니다. 쿨다운: {CD}.
* 티르's 수리 드론: 치유 드론을 배치하여 {**보조.x**}초 동안 {**주.x**}m 범위 내 아군의 장갑을 초당 2100 회복합니다. 재사용 대기시간: {CD}.
* 데커's 연쇄 과부하: 적 로봇을 바이러스로 감염시켜 6초 동안 초당 {**주.x**} 피해를 입힙니다. 효과가 지속되는 동안 다른 적이 {**보조.x**}m 범위 내로 들어오면 바이러스가 전이됩니다. 최대 8번까지 연쇄됩니다.
* 불가사리's 강철비: 해당 지역을 폭격하여 {**보조.x**}m 폭발 반경 내의 모든 모듈에 로켓 한 발당 {**주.x**} 피해를 입힙니다.
* 사이클롭스's 죽음의 광선: 짧은 지연 후 에너지 광선을 발사하여 {**보조.x**}초 동안 초당 {**주.x**}의 피해를 입힙니다. 활성화 중에는 로봇이 걸을 수 없습니다.
* 바랑기안's 에너지 돔: 적의 공격으로부터 최대 {**주.x**}의 피해를 흡수하는 {**보조.x**}m 반경의 장벽을 배치합니다.
* Artemis's 사냥꾼의 징표: 적중 시 적을 드러내는 발사체를 발사하여 {**보조.x**}초 동안 받는 아머 피해를 20% 증가시킵니다. 효과가 지속되는 동안 대상이 파괴되면, 지난 10초 내에 피해를 입힌 모든 아군이 각 아머 구역의 아머를 {**주.x**}만큼 회복합니다. 재사용 대기시간: {CD}.
* Relic Bulgasari's 아르페지오: 목표 구역을 상공에서 타격하는 로켓 일제 사격을 개시합니다. 각 로켓의 폭발 반경은 {**보조.x**}m이며, 폭발 범위 내의 모든 로봇 부품에 {**주.x**}의 피해를 줍니다. 재사용 대기시간: {CD}.
* 퓨리파이어's 광신도: 근처 아군이 {**주.x**}초 동안 {**보조.x**}%의 추가 피해를 입힙니다. 아군이 적을 처치하거나 실드를 파괴할 때마다 효과가 증가합니다.
* 로키's 홀로그램 미끼: {**주.x**}초 동안 은신 상태가 되며 로봇의 복제체를 생성합니다. 복제체는 아머와 실드가 {**보조.x**}% 더 높고, 기어 모듈을 포함한 모든 활성화된 효과가 복제체로 이전됩니다.
* Heike's 장갑 탈피: 70m 범위의 펄스로 보호막을 방출해 적 보호막에 {**주.x**} 피해를 주고, {**보조.x**}초 동안 Heike에게 30%, 범위 내 아군에게 15% 피해 저항을 부여합니다.
* 오로치's 에코 연발: 일직선상의 적에게 {**주.x**}의 실드 피해를 주고 {**보조.x**}초 동안 적을 드러냅니다.
* 펜리르's 라그나로크: 현재 장갑의 {**보조.Cx**}%을 희생해 10초 동안 피해를 25% 증가시킵니다. 가한 피해의 {**주.x**}%만큼 보호막과 구조를 회복합니다. 재사용 대기시간: {CD}.
* 티폰's 블랙아웃: 유도 미사일 여러 발을 발사하여 최대 3기의 적을 재장전하도록 만들고, {**보조.x**}초 동안 재장전 속도를 {**주.x**}% 늦춥니다.
* Garuda's 스네이크 캐처: 모듈을 발사하여 충돌 시 에너지 감옥을 전개하고 적 로봇의 통과를 막습니다. 감옥은 {**주.x**}초 동안 유지됩니다. 내부에 갇힌 적은 연사 속도와 무기 산포에 {**보조.x**}% 페널티를 받습니다.
* Wyrm's 리클레이머: 타겟에게 자석 작살을 발사하여 {**주.x**}초 동안 끌어당깁니다. 능력이 활성화된 동안, 로봇은 전방 구역에 있는 모든 적 로봇 부품에 초당 {**보조.x**}의 피해를 줍니다. 재사용 대기시간: {CD}.
* 레이븐's 강철 깃털: 전방에 미사일을 연발로 발사하여, 각각 {**보조.x**}m 폭발 반경으로 {**주.x**} 피해를 입힙니다.
* 팬텀's 위치전송기: 활성화하면 {**주.x**}초 동안 위치를 표시합니다. 다시 활성화하면 {**보조./**}초 지연 후 해당 위치로 순간이동합니다.
* 스콜피언's 채찍: 유도 투사체 명중 시 {**주.x**}의 피해를 주고 모든 모듈에 초당 {**보조.x**}의 피해를 입힙니다. 대상이 활성화된 상태에서 파괴되면 재충전됩니다.
* 하피's 네이팜 포탄: 투사체를 발사하여 명중 시 {**보조.x**} 폭발 피해를 입히고, 이후 {**주.x**} 화염 피해를 지속적으로 입힙니다.
* 불워크's 흡수 구체: {**주.x**}초 동안 받는 피해를 {**보조.x**}% 완화합니다. 구체에 맞은 투사체는 탄약을 보충합니다.
* 쿠모's 워프아머: 로봇이 선택한 위치로 순간이동하며, 자신과 주변 아군에게 {**주.x**}% 피해 감소 효과를 부여합니다. 효과는 {**보조.x**}초 동안 지속됩니다.
* 아난시's 위장 거미줄: {**주.x**}초 동안 {**보조.x**}m 반경에 위장 거미줄을 만들어 그 안의 아군이 바깥의 적에게 보이지 않게 합니다. 쿨다운 {CD}.
* 퓨리's 워프 재장전: {**주./**}초 지연 후 탄약의 {**보조.x**}%를 보충합니다.
* 퍼슈어's 광학 위장: {**보조./**}초 지연 후, {**주.x**}초 동안 은신 상태가 됩니다. 은신 상태에서도 기어 모듈을 사용할 수 있지만, 무기를 발사하면 즉시 로봇이 드러납니다. 쿨다운: {CD}.
* 아레스's 격노: 내장형 방벽 및 추가 무기를 활성화하여 {**보조.x**}초 동안 한 발당 {**주.x**} 피해를 입힙니다.
* 그리핀's 스크램블러: 적이 {**주.x**}m 내의 아군을 추적하지 못하게 하고, 받는 모든 피해를 모듈 간에 분산시킵니다. {**보조.x**}초 동안 지속됩니다. 영향받는 로봇은 파괴할 수 없습니다.
* 랜슬롯's 전투 명령: {**주.x**}m 내의 아군이 받는 피해가 {**보조.x**}% 감소합니다. 공격받을 때마다 아머가 재생됩니다.
* 라바나's 수호 매트릭스: {**주.x**}초 동안 전방의 아군이 받는 피해가 {**보조.x**}% 감소합니다. 이 효과를 받는 아군이 있는 동안에는 파괴되지 않습니다.
* 사이렌's 급속 냉각: 폭발 반경 내의 적에게 {**주.x**} 피해를 입히고 {**보조.x**}초 동안 이동 불가 상태로 만드는 동결 투사체를 발사합니다.
* Helios's 플레이밍 라이더: {**보조.x**}초 동안 활성화되어 로봇 아래에 나팜 웅덩이를 떨어뜨립니다. 웅덩이는 적 로봇을 30% 감속시키고 영향 범위 내의 각 로봇 부품에 초당 {**주.x**} 피해를 줍니다. 활성화된 동안 로봇은 언제든지 시작 위치로 순간이동하여 돌아갈 수 있습니다. 재사용 대기시간: {CD}.
* 크릭스's 그래플링: {**주.x**}초 동안 대상에 달라붙는 나노봇 군단을 방출하여 로봇을 대상 쪽으로 끌어당기고 {**보조.x**}의 피해를 입힙니다.
* 세레스's 회복기: 초당 아머의 {**주.x**}%를 수리해. {**보조.x**}m 내에 아군이 있으면 아군도 수리하고 수리 속도가 두 배가 돼. 12초 동안 지속돼.
* Relic Scorpion's 섀도 스트라이크: 로봇을 최대 {**주.x**}m까지 순간이동시킵니다. 재등장 시 범위 내의 모든 적에게 25000의 실드 피해를 줍니다. 또한 내장된 테일 캐논이 {**보조.x**}초 동안 활성화되어 초당 3000의 피해를 줍니다. 재사용 대기시간: {CD} resistance.
* Relic Lancelot's 팔라딘의 수호: {**주.x**}초 동안 피해 저항 35%를 부여합니다. 활성화 상태에서 주변 아군은 Lancelot이 흡수한 피해의 {**보조.x**}%만큼 아머를 회복합니다. 재사용 대기시간: {CD}.
* Hitcher's 날개 드론: 평평한 날개 길이 {**보조.x**}의 드론을 소환하고 경로의 모든 적에게 {**주.x**} 피해를 줍니다.

## 순환 기어

* 섬광탄: 유도 투사체를 발사하여 대상에게 {**보조.x**} 피해를 입히고 {**주.x**}초 동안 실명시킵니다.
* 정찰 모드: 로봇의 속도가 증가하고, {**보조.x**}m 범위 내의 모든 적을 {**주.x**}초 동안 드러냅니다.
* 나나이트 수리: 모든 모듈의 아머를 틱당 {**주.x**}% 수리합니다. {**보조.x**}초 동안 지속됩니다.
* 보급 급증: {**주./**}초간 예열하여, 충전 1개를 회복하고 보급 기어의 쿨다운을 {**보조.x**}%만큼 감소시킵니다. 필요 충전: {ChargeDrain}.
* 반사 장치: {**주.x**}초 동안 받은 피해의 {**보조.x**}%를 적에게 반사합니다.
* 무한 탄약: {**보조./**}초 지연 후, {**주.x**}초 동안 무제한 탄약을 제공합니다.
* 유도 미사일: 유도 미사일 한 발을 발사하여 폭발 범위의 모든 아머 부위에 {**주.x**} 피해 또는 실드에 {**보조.x**} 피해를 입힙니다.
* 감마 빔: {**보조.x**}초 동안 적을 조준하며, 초당 최대 {**주.x**} 피해를 입힙니다. 가까울수록 피해가 증가합니다.
* 지뢰밭: {**보조.x**}초 동안 지속되는 투명 지뢰밭을 배치합니다. 각 지뢰는 접촉 시 적을 둔화시키고 {**주.x**} 피해를 입힙니다.
* 우산: 상공의 공격을 차단하는 {**보조.x**}m 반경의 장벽을 배치합니다. {**주.x**}초 동안 지속됩니다.
* 강철 베일: {**주.x**}%만큼 받는 모든 피해를 {**보조.x**}초 동안 줄여. 필요한 충전: {ChargeDrain}.


# Primary and Secondary Parameters in Ability Descriptions (pt-BR)

## Fornecimento de equipamento

* Reserva de Combustível: Recarrega até {**Secundário.x**} unidades de combustível de combustível, {**Primário.x**} unidades de combustível unidades por ciclo.
* Queima de combustível: Queima {**Secundário.x**}% combustível do alvo e bloqueia sua regeneração por {**Primário.x**}s. Tempo de recarga {CD}, {Consum} cargas.
* Onda Explosiva: Planta uma bomba dentro de {**Secundário.x**}m. Após um pequeno atraso, a explosão causa {**Primário.x**} de dano e empurra os inimigos para trás.
* Supressor: Posiciona um emissor adesivo, que reduz o dano dos inimigos na área em {**Primário.x**}% por {**Secundário.x**}s.
* Reparos rápidos: Repara seu módulo mais danificado em {**Primário.x**}% da armadura por segundo. Dura {**Secundário.x**}s.
* Pulso Cinético: Dispara um projétil que se fixa a uma superfície ou a um robô e emite {**Primário.x**} pulsos, empurrando inimigos a uma distância de {**Secundário.x**}m para todos os lados. Reativação {CD}. {Consum} cargas.
* Piscadela: Após um atraso de {**Primário.x**}m, teletransporta {**Primário.x**}m na direção onde você apontar.
* Torreta Fantasma: Gera uma torreta que ataca o inimigo mais próximo e causa {**Primário.x**} por tiro durante {**Secundário.x**}s.
* Reator Veloz: Turbina sua velocidade máxima em {**Secundário.x**}km/h por {**Primário.x**}s.
* Escudo de Emergência: Restaura {**Primário.x**} de escudo por segundo por {**Secundário.x**}s.
* Fábrica de munição: Posiciona um dispositivo que reduz o tempo de recarga de aliados dentro de {**Secundário.x**}m em {**Primário.x**}%.
* Barreira de Plasma: Lança uma barreira de {**Primário.x**}m que para os inimigos e absorve até {**Secundário.x**} de dano.
* Repulsora: Causa {**Secundário.x**} de dano a inimigos, e empurra para longe qualquer um com uma força de {**Primário.x**}m/s.
* Bloqueio holográfico: Lança uma parede de {**Secundário.x**}m de largura, que bloqueia a linha de visão por {**Primário.x**}s.

## Torso

* Mesa's Harmonizador: Restaura {**Primário.x**} de escudo por segundo dos aliados dentro de {**Secundário.x**}m e metade disso para o próprio Mesa. Reduz o dano causado dos inimigos no alcance em 30%. Reativação: {CD}.
* Tyr's Drone de Reparo: Implanta um drone de cura que restaura 2100 de armadura por segundo aos aliados dentro de {**Primário.x**}m por {**Secundário.x**}s. Recarga: {CD}.
* Decker's Sobrecarga em Cadeia: Infecta uma máquina inimiga com um vírus que causa {**Primário.x**} de dano por segundo durante 6 segundos. Se outra máquina inimiga entrar no raio de {**Secundário.x**}m durante o efeito, o vírus se espalha para ela. Pode se encadear até 8 vezes.
* Bulgasari's Chuva de Balas: Bombardeia uma área, causando {**Primário.x**} de dano por foguete a todos os módulos dentro de um raio de explosão de {**Secundário.x**}m.
* Cyclops's Raio da Morte: Após um curto intervalo, dispara um feixe de energia que causa {**Primário.x**} de dano por segundo durante {**Secundário.x**}s. Seu robô não pode andar enquanto ela estiver ativa.
* Varangian's Domo de Energia: Posiciona uma barreira com um raio de {**Secundário.x**}m que absorve até {**Primário.x**} de dano dos ataques inimigos.
* Artemis's Marca do Caçador: Dispara um projétil que revela um inimigo ao atingir, aumentando o dano de armadura recebido em 20% por {**Secundário.x**}s. Se o alvo for destruído enquanto estiver afetado, todos os aliados que causaram dano a ele nos últimos 10 segundos restauram {**Primário.x**} de armadura em cada zona de armadura. Recarga: {CD}.
* Relic Bulgasari's Arpeggio: Lança uma salva de foguetes que atinge a área-alvo por cima. Cada foguete tem um raio de explosão de {**Secundário.x**}m e causa {**Primário.x**} de dano a cada componente de robô dentro da área da explosão. Tempo de recarga: {CD}.
* Purifier's Fanático: Aliados próximos causam {**Secundário.x**}% mais dano por {**Primário.x**}s. O efeito aumenta quando seus aliados abaterem robôs ou destruírem escudos.
* Loki's Isca Holográfica: Você fica invisível e projeta um cópia do seu Robô por {**Primário.x**}s. Essa cópia tem {**Secundário.x**}% a mais de armadura e escudo, e todos os efeitos ativos, incluindo os de módulos de equipamento, são transferidos para ela.
* Heike's Troca de Carapaça: Descarga seus escudos em um pulso de 70 m, causando {**Primário.x**} de dano aos escudos inimigos e concedendo 30% de resistência a dano para Heike e 15% para aliados na área por {**Secundário.x**}s segundos.
* Orochi's Barragem Eco: Causa {**Primário.x**} de dano a escudos em linha reta, revelando inimigos por {**Secundário.x**}s.
* Fenrir's Ragnarok: Sacrifica {**Secundário.Cx**}% da armadura atual para aumentar o dano em 25% por 10 segundos. Restaura escudo e estrutura em {**Primário.x**}% do dano causado. Recarga: {CD}.
* Typhon's Blecaute: Atira mísseis guiados que forçam até 3 inimigos a recarregar, além de desacelerar suas recargas em {**Primário.x**}% por {**Secundário.x**}s.
* Garuda's Caçador de Cobras: Dispara um módulo que, ao impactar, implanta uma jaula de energia que impede a passagem de robôs inimigos. A jaula dura {**Primário.x**}s. Inimigos presos sofrem uma penalidade de {**Secundário.x**}% na cadência de tiro e dispersão das armas.
* Wyrm's Reclamador: Dispara um arpão magnético em um alvo e o puxa por {**Primário.x**}s. Enquanto estiver ativo, o robô causa {**Secundário.x**} de dano por segundo a cada componente de robô inimigo na área à sua frente. Tempo de recarga: {CD}.
* Raven's Penas de Aço: Atira uma barreira de mísseis para frente, cada um causando {**Primário.x**} de dano com um raio de explosão de {**Secundário.x**}m.
* Phantom's Translocador: Ative para marcar um local por {**Primário.x**}s. Ative novamente para voltar para o local marcado depois de um atraso de {**Secundário./**}s.
* Scorpion's Chicotada: Um projétil guiado que causa {**Primário.x**} de dano ao acertar e mais {**Secundário.x**} por segundo a todos os módulos. Recarrega se o alvo for destruído enquanto estiver ativo.
* Harpy's Bala de Napalm: Atira um projétil que causa {**Secundário.x**} de dano de explosão no impacto e {**Primário.x**} de dano de fogo ao longo do tempo.
* Bulwark's Esfera de Absorção: Mitiga {**Secundário.x**}% do dano recebido por {**Primário.x**}s. Projéteis pegos na esfera recarregam sua munição.
* Kumo's Armadura das Dobras: O robô se teleporta para um local selecionado e aplica uma redução de dano de {**Primário.x**}% a si mesmo e aos aliados próximos. O efeito dura {**Secundário.x**}s.
* Anansi's Rede de Camuflagem: Cria uma rede de camuflagem com um raio de {**Secundário.x**}m por {**Primário.x**}s que torna os aliados invisiveis para os inimigos do lado de fora. Reativação: {CD}.
* Fury's Recarregar a Dobra: Recupera {**Secundário.x**}% da munição após um atraso de {**Primário./**}s.
* Pursuer's Camuflagem Óptica: Após um atraso de {**Secundário./**}s, você fica invisível por {**Primário.x**}s. Módulos de Equipamento podem ser usados enquanto invisível, mas atirar revela o robô imediatamente. Reativação: {CD}.
* Ares's Fúria: Ativa uma barreira embutida e armas extras que causam {**Primário.x**} de dano por tiro durante {**Secundário.x**}s.
* Griffin's Misturador: Impede que os inimigos travem nos aliados dentro de {**Primário.x**}m e divide todo o dano recebido entre os módulos. Dura {Duration}. Nenhum robô afetado pode ser destruído.
* Lancelot's Ordens de Batalha: Aliados dentro de {**Primário.x**}m sofrem {**Secundário.x**}% de dano a menos. Sempre que eles forem atingidos, sua armadura se regenera.
* Ravana's Matriz Guardiã: Aliados na sua frente sofrem {**Secundário.x**}% de dano a menos por {**Primário.x**}s. Você não pode morrer enquanto um aliado é afetado por isso.
* Siren's Tiro Congelante: Atira um projétil imobilizante que causa {**Primário.x**} de dano a inimigos no raio da explosão e os imobiliza por {**Secundário.x**}s.
* Helios's Cavaleiro Flamejante: Ativa por {**Secundário.x**}s, deixando poças de napalm sob o robô. As poças desaceleram os robôs inimigos em 30% e causam {**Primário.x**} de dano por segundo a cada componente de robô dentro da área afetada. Enquanto estiver ativa, o robô pode se teletransportar de volta à sua posição inicial a qualquer momento. Tempo de recarga: {CD}.
* Crix's Arpéu: Libera um enxame de nanobots que se fixam no alvo durante {**Primário.x**}s, puxando o robô em direção a ele e causando {**Secundário.x**} de dano.
* Ceres's Fonte da Juventude: Repara {**Primário.x**}% da armadura por segundo. Se houver aliados dentro de {**Secundário.x**}m, eles também recebem reparos e a taxa de reparo é dobrada. Dura 12 segundos.
* Relic Scorpion's Ataque das Sombras: Teleporta o robô em até {**Primário.x**}m. Ao reaparecer, causa 25000 de dano de escudo a todos os inimigos na área. Também ativa um canhão de cauda integrado por {**Secundário.x**}s, causando 3000 de dano por segundo. Tempo de recarga: {CD}.
* Relic Lancelot's Proteção do Paladino: Concede 35% de resistência a dano por {**Primário.x**}s. Enquanto estiver ativa, aliados próximos restauram armadura igual a {**Secundário.x**}% do dano absorvido por Lancelot. Tempo de recarga: {CD}.
* Carona's Drone Alado: Cria um drone com asas planas de {**Secundário.x**}, que causam {**Primário.x**} de dano aos inimigos em seu caminho.

## Equipamento de turno

* Granada de Luz: Atira um projétil guiado que causa {**Secundário.x**} de dano e cega o alvo por {**Primário.x**}s.
* Modo de Reconhecimento: O Robô ganha um impulso de velocidade e revela todos os inimigos dentro de {**Secundário.x**}m por {**Primário.x**}s.
* Nano Reparadores: Repara todos os módulos em {**Primário.x**}% da armadura por ciclo. Dura {**Secundário.x**}s.
* Pulso de Suprimentos: Após uma preparação de {**Primário./**}s, restaura 1 carga e reduz o tempo de recarga do Fornecimento de equipamento em {**Secundário.x**}%. Carga necessária: {ChargeDrain}.
* Refletor: Devolve {**Secundário.x**}% do dano recebido para o inimigo durante {**Primário.x**}s.
* Munição Infinita: Após {**Secundário./**}s da ativação, concede munição ilimitada por {**Primário.x**}s.
* Míssil Guiado: Atira um míssil guiado que causa {**Primário.x**} de dano a qualquer parte de armadura na área da explosão ou {**Secundário.x**} a escudos.
* Onda Gamma: Trava no inimigo por {**Secundário.x**}s, causando até {**Primário.x**} de dano por segundo. O dano aumenta com a proximidade.
* Campo Minado: Posiciona um campo minado invisível por {**Secundário.x**}s. Cada mina desacelera os inimigos ao explodir e causa {**Primário.x**} de dano.
* Guarda-Chuva: Lança uma barreira com um raio de {**Secundário.x**}m, que protege contra ataques de cima. Dura por {**Primário.x**}s.
* Véu de Ferro: Reduz todo dano recebido em {**Primário.x**}% por {**Secundário.x**}s. Carga necessária: {ChargeDrain}.


# Primary and Secondary Parameters in Ability Descriptions (ja)

## サプライギア

* 燃料予備: 燃料が1カウントあたり{**メイン.x**}燃料ユニット、最大{**サブ.x**}燃料ユニットまで補充される。
* 燃料燃焼: ターゲットから{**サブ.x**}%の燃料を燃やし、{**メイン.x**}秒間その回復をブロックする。クールダウン：{CD}、チャージ：{Consum}。
* ブラストウェーブ: {**サブ.x**}m以内に爆弾を設置する。しばらくすると爆発して{**メイン.x**}ダメージを与え、敵をノックバックさせる。
* サプレッサー: 範囲内の敵のダメージを{**サブ.x**}秒間、{**メイン.x**}%軽減する粘着性エミッターを展開する。
* クイック修復: 最も損傷したモジュールの装甲を1秒あたり{**メイン.x**}%修復する。{**サブ.x**}秒間持続する。
* 運動パルス: 表面や機体に付着し、半径{**サブ.x**}m内の敵を四方八方に吹き飛ばすパルスを{**メイン.x**}回放出する発射物を撃ち出す。クールダウン {CD}。{Consum}チャージ。
* ブリンク: {**サブ./**}秒後、向いている方向に{**メイン.x**}mテレポートする。
* ゴーストタレット: 最も近い敵を攻撃する砲塔を展開し、1発あたり{**メイン.x**}ダメージを{**サブ.x**}秒にわたって与える。
* スプリントリアクター: {**メイン.x**}秒間、最大速度が{**サブ.x**}km/hブーストされる。
* 緊急シールド: {**メイン.x**}間、1秒あたり{**サブ.x**}秒シールドを回復する。
* 弾薬ファブリケーター: {**サブ.x**}m以内の味方のリロード時間を{**メイン.x**}%短縮するデバイスを展開する。
* エネルギーウォール: 敵の動きを止め、最大{**サブ.x**}ダメージを吸収する{**メイン.x**}m幅のバリアを展開する。
* リパルサー: 敵に{**サブ.x**}ダメージを与え、{**メイン.x**}m/sの力で敵とプレイヤーをお互いに押し出す。
* ホロフィールド: {**メイン.x**}秒間、視線を遮る{**サブ.x**}m幅の壁を展開する。

## 胴体

* Mesa's ハーモナイザー: {**サブ.x**}m以内の味方のシールドを毎秒{**メイン.x**}回復し、Mesa自身はこの半分の値を回復する。範囲内の敵へのダメージ出力も30%減少する。 クールダウン: {CD}.
* Tyr's 修復ドローン: 回復ドローンを展開し、{**サブ.x**}秒 の間、{**メイン.x**}m 内の味方の装甲を毎秒 2100 回復する。クールダウン：{CD}。
* デッカー's 連鎖オーバーロード: 敵機をウイルスに感染させ、毎秒{**メイン.x**}ダメージを6秒間与える。効果中に別の敵機が{**サブ.x**}m以内に入るとウイルスが伝染し、最大8回まで連鎖する。
* Bulgasari's 鉄の雨: エリアを砲撃し、爆発半径{**サブ.x**}m以内のすべてのモジュールにロケット1発につき{**メイン.x**}ダメージを与える。
* Cyclops's デス・レイ: 短い遅延の後、エネルギービームを放ち、{**サブ.x**}秒の間、1秒あたり{**メイン.x**}のダメージを与える。発動中、ロボットは歩行できない。
* Varangian's エネルギードーム: 半径{**サブ.x**}m のバリアを展開し、敵の攻撃から最大{**メイン.x**}ダメージを吸収する。
* Artemis's ハンターの印: 命中した敵を露呈させる発射体を発射し、{**サブ.x**}秒の間、受けるアーマーダメージを20%増加させる。効果中にターゲットが破壊された場合、過去10秒以内にダメージを与えたすべての味方は、各アーマーゾーンのアーマーを{**メイン.x**}回復する。クールダウン: {CD}。
* Relic Bulgasari's アルペジオ: 目標エリアを上空から強襲するロケット弾幕を発射する。各ロケットは{**サブ.x**}mの爆発半径を持ち、爆発エリア内の敵ロボットの各パーツに{**メイン.x**}のダメージを与える。クールダウン：{CD}。
* Purifier's ジーロット: 近くの味方が与えるダメージが{**メイン.x**}秒間、{**サブ.x**}%増加する。キルするかシールドを破壊するたびに効果が増大する。
* Loki's ホログラフィックデコイ: {**メイン.x**}秒デコイの装甲とシールドは{**サブ.x**}%高い。デコイの装甲とシールドは{**サブ.x**}%増加し、ギアモジュールを含むすべてのアクティイブな効果がデコイに移行される。
* Heike's 装甲脱皮: シールドを70m範囲のパルスとして放出し、敵シールドに {**メイン.x**} ダメージを与える。さらに {**サブ.x**}秒 秒間、Heikeに30%のダメージ耐性、範囲内の味方に15%のダメージ耐性を付与する。
* Orochi's エコー弾幕: 直線上に{**メイン.x**}シールドダメージを与え、敵を{**サブ.x**}秒間発見状態にする。
* Fenrir's ラグナロク: 現在の装甲の {**サブ.Cx**}% を消費し、10秒間ダメージが25％増加する。与えたダメージの {**メイン.x**}% 分、シールドと構造を回復する。クールダウン：{CD}。
* Typhon's ブラックアウト: 最大3体の敵にリロードを強制する誘導ミサイルを発射し、同時に{**サブ.x**}秒間、リロード速度を{**メイン.x**}%遅くする。
* Garuda's スネークキャッチャー: モジュールを発射し、着弾時にエネルギーケージを展開して敵ロボットの通過を防ぐ。ケージは {**メイン.x**}秒 持続する。内部に閉じ込められた敵は射撃速度と武器の拡散が {**サブ.x**}% 低下する。
* Wyrm's リクレイマー: ターゲットに向けて磁気ハープーンを発射し、{**メイン.x**}秒の間引き寄せる。発動中、ロボットは前方エリア内にいる敵ロボットの各パーツに対して1秒あたり{**サブ.x**}のダメージを与える。クールダウン：{CD}。
* Raven's 鋼鉄の羽根: 前方にミサイルの弾幕を発射する。それぞれのミサイルは{**サブ.x**}mの爆発半径で{**メイン.x**}ダメージを与える。
* Phantom's トランスロケーター: アクティブにすると{**メイン.x**}秒間その場所をマークする。再度アクティブにすると{**サブ./**}秒後にその場所にワープする。
* Scorpion's むち打ち: 誘導式発射体は着弾時に{**メイン.x**}ダメージを与え、すべてのモジュールに1秒あたり{**サブ.x**}ダメージを与える。アクティブな間、ターゲットが破壊された場合はリチャージされる。
* Harpy's ナパーム弾: 着弾時に{**サブ.x**}のブラストダメージを与え、時間の経過ととも{**メイン.x**}の火災ダメージを与える発射体を発射する。
* Bulwark's アブソーバースフィア: 受けるダメージを{**メイン.x**}秒間、{**サブ.x**}%軽減する。球体に捕らえられた発射体がプレイヤーの弾薬を補充する。
* クモ's ワープアーマー: ロボットは指定地点にテレポートし、自身および周囲の味方に {**メイン.x**}% のダメージ軽減効果を付与する。効果は {**サブ.x**}秒 持続する。
* アナンシ's カモフラージュウェブ: 半径{**サブ.x**}mのカモフラージュウェブを{**メイン.x**}秒間生み出し、ウェブ内の味方を外部の敵から見えなくする。クールダウン：{CD}。
* Fury's ワープリロード: {**メイン./**}秒後に{**サブ.x**}%の弾薬が補充される。
* Pursuer's 光学カモフラージュ: {**サブ./**}秒秒後に{**メイン.x**}秒間、透明になる。ギアモジュールは透明な間も使用できるが、武器を発射するとロボットがすぐに見えるようになる。クールダウン：{CD}。
* Ares's 激怒: 内蔵バリアと追加の武器をアクティブにし、1発あたり{**メイン.x**}ダメージを{**サブ.x**}秒間与える。
* Griffin's スクランブラー: 敵が{**メイン.x**}m以内の味方にロックオンするのを防ぎ、被ダメージをすべてのモジュール間で分割する。{**サブ.x**}秒間持続する。影響を受けたロボットは破壊されない。
* Lancelot's 戦闘命令: {**メイン.x**}m以内の味方が受けるダメージが{**サブ.x**}%減少する。攻撃を受けるたびに装甲を修復する。
* Ravana's ガーディアンマトリクス: 目の前の味方が受けるダメージが{**メイン.x**}秒間、{**サブ.x**}%減少する。味方がこれによって影響を受けている間は破壊されない。
* Siren's コールドスナップ: 凍結弾を発射し、爆発半径内の敵に{**メイン.x**}ダメージを与え、{**サブ.x**}秒間動けなくする。
* Helios's フレイミングライダー: {**サブ.x**}秒の間発動し、ロボットの足元にナパームの溜まりを落とす。溜まりは敵ロボットを30%減速させ、効果範囲内の各ロボットパーツに毎秒{**メイン.x**}ダメージを与える。発動中、ロボットはいつでも開始位置にテレポートして戻ることができる。クールダウン：{CD}。
* クリクス's グラップリング: ナノボットの群れを放ち、{**メイン.x**}秒にかけてターゲットにラッチ（固定）し、ロボットをターゲットに引き寄せながら{**サブ.x**}のダメージを与える。
* Ceres's リジュビネーター: 装甲を1秒あたり{**メイン.x**}%修復する。{**サブ.x**}m内に味方がいる場合、その味方も修復し、修復率が2倍になる。12秒間持続する。
* Relic Scorpion's シャドウ・ストライク: ロボットを最大{**メイン.x**}mまでテレポートさせる。再出現時、エリア内のすべての敵に25000のシールドダメージを与える。また、内蔵のテールキャノンが{**サブ.x**}秒の間起動し、毎秒3000ダメージを与える。クールダウン：{CD}。
* Relic Lancelot's パラディンズ・ウォード: {**メイン.x**}秒の間、ダメージ耐性が35%向上する。発動中、周囲の味方はLancelotが吸収したダメージの{**サブ.x**}%に相当するアーマーを回復する。クールダウン：{CD}。
* ヒッチャー's ウイングドローン: 平らで{**サブ.x**}の長さの翼を持ったドローンを出現させ、進路上のすべての敵に{**メイン.x**}ダメージを与える。

## サイクルギア

* フラッシュバン: {**サブ.x**}ダメージを与え、ターゲットを{**メイン.x**}秒間盲目にする誘導式発射体を発射する。
* リーコンモード: {**メイン.x**}秒間、ロボットがスピードブーストを獲得し、{**サブ.x**}m以内のすべての敵を発見状態にする。
* ナナイト修復: すべてのモジュールの装甲を1カウントごとに{**メイン.x**}%修復する。{**サブ.x**}秒間持続する。
* 供給ラッシュ: {**メイン./**}秒の準備後、チャージを1補充し、サプライギアのクールダウンを{**サブ.x**}%短縮します。消費チャージ: {ChargeDrain}。
* リフレクター: {**メイン.x**}秒間、被ダメージの{**サブ.x**}%を敵に返す。
* 無限弾薬: {**サブ./**}秒後に{**メイン.x**}秒間、弾薬が無限になる。
* 誘導ミサイル: 爆発エリア内のすべての装甲セグメントに{**メイン.x**}、またはシールドに{**サブ.x**}ダメージを与える誘導ミサイルを発射する。
* ガンマビーム: {**サブ.x**}秒間、敵をロックオンし、1秒あたり最大{**メイン.x**}ダメージを与える。ダメージは距離に応じて変化する。
* 地雷原: {**サブ.x**}秒間持続する目に見えない地雷原を展開する。それぞれの地雷は命中時に敵を減速させ、{**メイン.x**}ダメージを与える。
* アンブレラ: 半径{**サブ.x**}mのバリアを展開し、上空からの攻撃を防ぐ。{**メイン.x**}秒間持続する。
* 鉄のベール: すべての受けるダメージが{**サブ.x**}秒間、{**メイン.x**}%減少する。必要チャージ：{ChargeDrain}。


# Primary and Secondary Parameters in Ability Descriptions (tr)

## İkmal Teçhizatı

* Yakıt Rezervi: Toplamda en fazla {**İkincil.x**}yakıt birimi olmak üzere döngü başına {**Birincil.x**}yakıt birimi birim yakıt doldurur.
* Yakıt Yakımı: Hedefin {**İkincil.x**}% yakıtını yakar ve onun yenilenmesini {**Birincil.x**}sn boyunca engeller. Bekleme süresi: {CD}. {Consum} şarj tüketir.
* İnfilak Dalgası: {**İkincil.x**}m Menzilli bir bomba yerleştirir. Kısa bir gecikmeden sonra patlama {**Birincil.x**} hasar verip düşmanları geriye iter.
* Baskılayıcı: Yapışkan bir yayıcı yerleştirir, bu da düşmanların alandaki hasarını {**İkincil.x**}sn boyunca {**Birincil.x**}% azaltır.
* Hızlı Onarım: En hasarlı modülünü saniyede %{**Birincil.x**} zırh onarır. {**İkincil.x**}sn sürer.
* Kinetik Darbe: Bir yüzeye veya robota yapışan bir mermi ateşler. Bu mermi, {**Birincil.x**} darbe yayarak {**İkincil.x**}m menzildeki düşmanları farklı yönlere savurur. Bekleme Süresi {CD}. {Consum} şarj.
* Parıltı: {**İkincil./**}sn gecikme ardından önüne doğru {**Birincil.x**}m öteye teleport olursun.
* Hayalet Taret: En yakın düşmana {**İkincil.x**}sn saniye boyunca atış başına {**Birincil.x**} hasar vererek saldıran bir taret konuşlandırır.
* Koşu Reaktörü: Maksimum hızını {**Birincil.x**}sn boyunca {**İkincil.x**}km/sa. artırır.
* Acil Durum Kalkanı: {**İkincil.x**}sn boyunca saniyede {**Birincil.x**} kalkan yeniler.
* Cephane Üretici: {**İkincil.x**}m dahilindeki müttefiklerin silah doldurma süresini {**Birincil.x**}% kısaltan bir cihaz konuşlandırır.
* Enerji Suru: Düşmanları durduran ve en fazla {**İkincil.x**} hasar soğuran {**Birincil.x**}m genişliğinde bir duvar konuşlandırır.
* Püskürtücü: Düşmanlara {**İkincil.x**} hasar verir, seni ve onları {**Birincil.x**}m/s kuvvetle birbirinizden öteye iter.
* Holo Saha: {**Birincil.x**}sn boyunca görüş hattını bloke eden {**İkincil.x**}m genişliğinde bir duvar konuşlandırır.

## Gövde

* Mesa's Harmonizör: {**İkincil.x**}m dahilindeki müttefikler için {**Birincil.x**}, Mesa için de bunu yarısı kadar kalkan yeniler. Ayrıca, menzil dahilindeki düşmanların verdiği hasarı %30 azaltır. Bekleme Süresi: {CD}.
* Tyr's Onarım Dronu: Bir iyileştirme dronu konuşlandırır ve {**İkincil.x**}sn boyunca {**Birincil.x**}m içindeki müttefiklere saniye başına 2100 zırh yeniler. Bekleme süresi: {CD}.
* Decker's Zincirleme Aşırı Yük: Düşman bir robotu, 6 saniye boyunca saniye başına {**Birincil.x**} hasar veren bir virüsle enfekte eder. Etki süresince {**İkincil.x**}m menziline başka bir düşman girerse virüs ona da sıçrar. Bu etki 8 defaya kadar zincirlenebilir.
* Bulgasari's Demir Yağmur: Bir alanı bombalar, {**İkincil.x**}m yarıçaplı bir patlama alanı içindekilere roket başına tüm modüllere {**Birincil.x**} hasar verir.
* Cyclops's Ölüm Işını: Kısa bir gecikmenin ardından, {**İkincil.x**}sn boyunca saniyede {**Birincil.x**} hasar veren bir enerji ışını yayar. Robotunuz bu yetenek etkinken yürüyemez.
* Varangian's Enerji Kubbesi: En fazla {**Birincil.x**} hasar soğuran {**İkincil.x**}m yarıçapı olan bir bariyer konuşlandırır.
* Artemis's Avcı Nişanı: Çarptığında bir düşmanı açığa çıkaran bir mermi ateşler ve {**İkincil.x**}sn boyunca alınan zırh hasarını %20 artırır. Hedef bu etkideyken yok edilirse, son 10 saniye içinde ona hasar veren tüm müttefikler her zırh bölgesinde {**Birincil.x**} zırh yeniler. Bekleme süresi: {CD}.
* Relic Bulgasari's Arpeggio: Hedeflenen bölgeyi yukarıdan vuran bir roket salvosu ateşler. Her roketin patlama yarıçapı {**İkincil.x**}m'tur ve patlama alanı içindeki her bir robot parçasına {**Birincil.x**} hasar verir. Bekleme süresi: {CD}.
* Purifier's Zelot: Yakındaki müttefikler {**Birincil.x**}sn boyunca {**İkincil.x**}% daha çok hasar verir. Öldürme skoru yaptıklarında ya da kalkan imha ettiklerinde etki artar.
* Loki's Holografik Yem: Seni görünmez yapar ve {**Birincil.x**}sn boyunca robotunun bir kopyasını yansıtır. Yemin {**İkincil.x**}% daha fazla zırhı ve kalkanı vardır, teçhizat modüllerinden gelenler dahil tüm aktif etkiler de ona aktarılır.
* Heike's Zırh Atımı: Kalkanlarını 70 m’lik bir darbe halinde boşaltarak düşman kalkanlarına {**Birincil.x**} hasar verir ve {**İkincil.x**}sn saniye boyunca Heike’e %30, alandaki müttefiklere %15 hasar direnci kazandırır.
* Orochi's Yankı Salvosu: Düz bir hat boyunca {**Birincil.x**} zırh hasrı verir, düşmanları {**İkincil.x**}sn boyunca ortaya çıkarır.
* Fenrir's Ragnarok: Mevcut zırhın {**İkincil.Cx**}% kadarını feda ederek hasarı 10 saniyeliğine %25 artırır. Verilen hasarın {**Birincil.x**}% kadarı kadar kalkan ve yapı yeniler. Bekleme süresi: {CD}.
* Typhon's Karartma: 3’e kadar düşmanı silah doldurmaya zorlayan, bir yandan da silah doldurma hızlarını {**İkincil.x**}sn boyunca {**Birincil.x**}% yavaşlatan güdümlü füzeler atar.
* Garuda's Yılan Avcısı: Çarpma anında enerji kafesi açan bir modül ateşler ve düşman robotların geçmesini engeller. Kafes {**Birincil.x**}sn sürer. İçeride kalan düşmanlar atış hızı ve silah saçılmasına {**İkincil.x**}% ceza alır.
* Wyrm's Geri Kazanıcı: Bir hedefe manyetik zıpkın fırlatır ve onu {**Birincil.x**}sn boyunca kendine doğru çeker. Yetenek aktifken robot, önündeki alanda bulunan her bir düşman robot parçasına saniye başına {**İkincil.x**} hasar verir. Bekleme süresi: {CD}.
* Raven's Çelik Tüyler: Tam önüne doğru bir füze salvosu yapar. Füzelerin her biri {**İkincil.x**}m patlama yarıçaplı {**Birincil.x**} hasar verir.
* Phantom's Yer Değiştirici: Etkinleştirince bir noktayı {**Birincil.x**}sn boyunca işaretler. Tekrar etkinleştirince {**İkincil./**}sn gecikme ardından o noktaya ışınlanırsın.
* Scorpion's Kamçı Vuruşu: Anında {**Birincil.x**} hasar, sonra da tüm modüllere saniyede {**İkincil.x**} hasar veren bir güdümlü mermi. Etkinken hedef imha olursa yeniden şarj olur.
* Harpy's Napalm Fişeği: Anında {**İkincil.x**} patlama hasarı sonra da süreye yayılan {**Birincil.x**} ateş hasarı veren bir mermi atar.
* Bulwark's Soğurma Küresi: {**Birincil.x**}sn boyunca alınan hasarı {**İkincil.x**}% oranında engeller. Küreye yakalanan mermiler cephaneni doldurur.
* Kumo's Işın Zırhı: Robot seçilen bir noktaya ışınlanır ve kendisine ve yakındaki müttefiklere {**Birincil.x**}% hasar azaltma etkisi uygular. Etki {**İkincil.x**}sn sürer.
* Anansi's Kamuflaj Ağı: {**Birincil.x**}sn boyunca {**İkincil.x**}m yarıçaplı bir Kamuflaj Ağı oluşturur, içindeki müttefikleri dışarıdaki düşmanlara görünmez kılar. Bekleme süresi: {CD}.
* Fury's Işın Doldurma: {**Birincil./**}sn gecikme ardından {**İkincil.x**}% cephane doldurur.
* Pursuer's Optik Kamuflaj: {**İkincil./**}sn gecikmeden sonra {**Birincil.x**}sn boyunca görünmez olursun. Teçhizat modülleri görünmezken kullanılabilse de ateşli silahlar robotu anında ortaya çıkarır. Bekleme süresi: {CD}.
* Ares's Gazap: {**İkincil.x**}sn boyunca dahili bir bariyer ve atış başına {**Birincil.x**} hasar veren ek silahlar etkinleştirir.
* Griffin's Çırpıcı: Düşmanların {**Birincil.x**}m dahilindeki müttefikler üzerinde kilitlenmesini önler ve alınan hasarı tüm modüllere bölüştürür. {**İkincil.x**}sn sürer. Etkilenen robotlar imha edilemez.
* Lancelot's Muharebe Düzeni: {**Birincil.x**}m dahilindeki müttefikler {**İkincil.x**}% daha az hasar alır. Vurulduklarında zırh yenilersin.
* Ravana's Muhafız Matriksi: Önündeki müttefikler {**Birincil.x**}sn boyunca {**İkincil.x**}% daha az hasar alır. Bir müttefik bu etkinin altındaysa öldürülemezsin.
* Siren's Soğuk Kapış: Patlama alanında kalan düşmanlara {**Birincil.x**} hasar veren ve onları {**İkincil.x**}sn boyunca hareketsiz kılan bir dondurucu mermi atar.
* Helios's Alevli Sürücü: {**İkincil.x**}sn boyunca aktifleşerek robotun altına napalm birikintileri bırakır. Birikintiler düşman robotları %30 yavaşlatır ve etki alanındaki her robot parçasına saniye başına {**Birincil.x**} hasar verir. Aktifken robot, istediği zaman başlangıç konumuna geri ışınlanabilir. Bekleme süresi: {CD}.
* Crix's Bordalama: Hedefe {**Birincil.x**}sn boyunca kenetlenen bir nanobot sürüsü fırlatır, robotu hedefe doğru çeker ve {**İkincil.x**} hasar verir.
* Ceres's Rejüvenatör: Saniyede %{**Birincil.x**} zırh onarır. {**İkincil.x**}m dahilinde müttefik varsa onlara da onarım yapar ve onarım oranını ikiye katlar. 12 saniye sürer.
* Relic Scorpion's Gölge Vuruşu: Robotu {**Birincil.x**}m mesafeye kadar ışınlar. Yeniden belirdiğinde alandaki tüm düşmanlara 25000 kalkan hasarı verir. Ayrıca entegre kuyruk topunu {**İkincil.x**}sn boyunca etkinleştirerek saniye başına 3000 hasar verir. Bekleme süresi: {CD}.
* Relic Lancelot's Paladin'in Muhafızı: {**Birincil.x**}sn boyunca %35 hasar direnci sağlar. Aktifken, yakındaki müttefikler Lancelot tarafından emilen hasarın {**İkincil.x**}% kadarına eşit zırh yeniler. Bekleme süresi: {CD}.
* Otostopçu's Kanatlı Dron: {**İkincil.x**} uzunluğunda düz kanatlara sahip bir dron oluşturur ve yoluna çıkan tüm düşmanlara {**Birincil.x**} hasar verir.

## Devir Teçhizatı

* Flaş Bombası: {**İkincil.x**} hasar veren ve hedefi {**Birincil.x**}sn boyunca kör eden bir güdümlü füze atar.
* Keşif Modu: Robot bir hız desteği kazanır ve {**İkincil.x**}m dahilindeki tüm düşmanları {**Birincil.x**}sn boyunca ifşa eder.
* Nanit Onarımı: Tüm modüller döngü başına %{**Birincil.x**} zırh yeniler. {**İkincil.x**}sn sürer.
* Malzeme Akını: {**Birincil./**}sn hazırlık sonrasında, 1 Şarj yeniler ve İkmal Teçhizatı'nın bekleme süresini {**İkincil.x**}% azaltır. Şarj Maliyeti: {ChargeDrain}.
* Yansıtıcı: {**Birincil.x**}sn boyunca alınan hasarı {**İkincil.x**}% oranında düşmana geri yansıtır.
* Sonsuz Cephane: {**İkincil./**}sn gecikmeden sonra {**Birincil.x**}sn boyunca sonsuz cephane verir.
* Güdümlü Füze: Patlama alanındaki tüm zırh parçalarına {**Birincil.x**} veya kalkanlara {**İkincil.x**} hasar veren bir güdümlü füze atar.
* Gama Işını: {**İkincil.x**}sn Boyunca düşman üzerine kilitlenerek saniyede en fazla {**Birincil.x**} hasar verir. Hasar uzaklığa göre ölçeklenir.
* Mayın Tarlası: {**İkincil.x**}sn süren görünmez bir mayın tarlası konuşlandırır. Her mayın temas ettiği düşmanlara {**Birincil.x**} hasar verir ve onları yavaşlatır.
* Şemsiye: Yukarıdan gelen saldırılardan koruyan {**İkincil.x**}m yarıçaplı bir bariyer konuşlandırır. {**Birincil.x**}sn sürer.
* Demir Peçe: Alınan tüm hasarı {**İkincil.x**}sn boyunca {**Birincil.x**}% azaltır. Gereken şarj: {ChargeDrain}.


# Primary and Secondary Parameters in Ability Descriptions (fr)

## Ravitaillement

* Réserve de carburant: Vous redonne jusqu'à {**Secondaire.x**} unités de carburant à raison de {**Primaire.x**} unités de carburant par cycle.
* Combustion: Brûle {**Secondaire.x**} % de carburant de la cible et bloque sa régénération pendant {**Primaire.x**} s. Temps de recharge : {CD}, {Consum} charges.
* Onde de choc: Pose une bombe dans un rayon de {**Secondaire.x**} m. Après un court délai, l'explosion inflige {**Primaire.x**} points de dégâts et repousse les ennemis.
* Suppresseur: Déploie un émetteur collant qui réduit les dégâts des ennemis dans la zone de {**Primaire.x**} % pendant {**Secondaire.x**} s.
* Réparation rapide: Répare votre module le plus endommagé à raison de {**Primaire.x**} points de blindage par seconde. Dure {**Secondaire.x**} s.
* Impulsion Cinétique: Lance un projectile qui adhère à une surface ou à un méca et émet {**Primaire.x**} impulsions, repoussant les ennemis dans différentes directions sur un rayon de {**Secondaire.x**} m. Temps de recharge {CD}. {Consum} unités.
* Clignement: Après un délai de {**Secondaire./**} s, vous téléporte sur {**Primaire.x**} m droit devant vous.
* Tourelle fantôme: Déploie une tourelle qui inflige à l'ennemi le plus proche {**Primaire.x**} points de dégâts par tir pendant {**Secondaire.x**} s.
* Réacteur de sprint: Booste votre vitesse maximum de {**Secondaire.x**} km/h pendant {**Primaire.x**} s.
* Bouclier d'urgence: Restaure {**Primaire.x**} points de bouclier par seconde pendant {**Secondaire.x**} s.
* Fabricant de munitions: Déploie un appareil qui réduit la vitesse de recharge des alliés dans les {**Secondaire.x**} m m de{**Primaire.x**} %.
* Mur d'énergie: Déploie une barrière large de {**Primaire.x**} m qui arrête les ennemis et absorbe jusqu'à {**Secondaire.x**} points de dégâts.
* Répulseur: Inflige {**Secondaire.x**} points de dégâts aux ennemis et les repousse loin de vous avec une force de {**Primaire.x**}m/s.
* Champ holographique: Déploie un mur large de {**Secondaire.x**} m qui bloque la vue pendant {**Primaire.x**} s.

## Torse

* Mesa's Harmonisateur: Restaure {**Primaire.x**} points de bouclier par seconde pour les alliés dans les {**Secondaire.x**} m, et la moitié de cette valeur pour le Mesa. Réduit aussi les dégâts infligés par les ennemis à portée de 30 %. Temps de recharge: {CD}.
* Tyr's Drone réparateur: Déploie un drone de soin qui restaure 2100 points d’armure par seconde aux alliés dans un rayon de {**Primaire.x**} m pendant {**Secondaire.x**} s. Recharge : {CD}.
* Decker's Surcharge en cascade: Infecte un mécha ennemi avec un virus qui inflige {**Primaire.x**} DPS pendant 6 s. Si un autre ennemi s'approche à moins de {**Secondaire.x**} m durant l'effet, le virus se propage à lui. Peut s'enchaîner jusqu'à 8 fois.
* Bulgasari's Pluie de fer: Bombarde une zone, infligeant {**Primaire.x**} points de dégâts par roquette à tous les modules dans un rayon d'explosion de {**Secondaire.x**} m.
* Cyclops's Rayon mortel: Après un court délai, libère un faisceau d'énergie qui inflige {**Primaire.x**} points de dégâts par seconde pendant {**Secondaire.x**} s. Votre robot ne peut pas marcher lorsqu'il est actif.
* Varangian's Dôme d'énergie: Déploie une barrière d'un rayon de {**Secondaire.x**} m qui absorbe jusqu'à {**Primaire.x**} points de dégâts des attaques ennemies.
* Artemis's Marque du chasseur: Tire un projectile qui révèle un ennemi à l'impact, augmentant les dégâts d'armure subis de 20 % pendant {**Secondaire.x**} s. Si la cible est détruite alors qu'elle est affectée, tous les alliés l'ayant endommagée au cours des 10 dernières secondes restaurent {**Primaire.x**} points d'armure dans chaque zone d'armure. Temps de recharge : {CD}.
* Relic Bulgasari's Arpeggio: Lance une salve de roquettes qui frappent la zone ciblée par le haut. Chaque roquette a un rayon d'explosion de {**Secondaire.x**} m et inflige {**Primaire.x**} points de dégâts à chaque élément de robot dans la zone d'impact. Temps de recharge : {CD}.
* Purifier's Zélote: Vos alliés proches infligent {**Secondaire.x**} % points de dégâts en plus pendant {**Primaire.x**} s. L'effet augmente s'ils font des victimes ou détruisent des boucliers.
* Loki's Leurre holographique: Vous rend invisible et projette une copie de votre robot pendant {**Primaire.x**} s. Le blindage et les boucliers de votre leurre sont accrus de {**Secondaire.x**} %, et tous les effets actifs, y compris ceux des modules de mécanisme, lui sont transférés.
* Heike's Mue défensive: Décharge ses boucliers dans une impulsion de 70 m, infligeant {**Primaire.x**} aux boucliers ennemis et accordant 30 % de résistance aux dégâts à Heike et 15 % aux alliés dans la zone pendant {**Secondaire.x**} s secondes.
* Orochi's Barrage à écho: Inflige {**Primaire.x**} points de dégâts au bouclier en ligne droite et révèle les ennemis pendant {**Secondaire.x**} s.
* Fenrir's Ragnarok: Sacrifie {**Secondaire.Cx**} % de l’armure actuelle pour augmenter les dégâts de 25 % pendant 10 secondes. Restaure bouclier et structure à hauteur de {**Primaire.x**} % des dégâts infligés. Recharge : {CD}.
* Typhon's Trou noir: Tire des missiles à tête chercheuse qui forcent jusqu'à 3 ennemis à recharger et diminuent leur vitesse de recharge de {**Primaire.x**} % pendant {**Secondaire.x**} s.
* Garuda's Attrape-serpent: Tire un module qui déploie une cage d’énergie à l’impact, empêchant les robots ennemis de la traverser. La cage dure {**Primaire.x**} s. Les ennemis piégés subissent une pénalité de {**Secondaire.x**} % à la cadence de tir et à la dispersion des armes.
* Wyrm's Récupérateur: Tire un harpon magnétique sur une cible et l'attire pendant {**Primaire.x**} s. Tant qu'il est actif, le robot inflige {**Secondaire.x**} points de dégâts par seconde à chaque élément de robot ennemi situé dans la zone devant lui. Temps de recharge : {CD}.
* Raven's Plumes d'acier: Tire un barrage de missiles droit devant qui infligent chacun {**Primaire.x**} points de dégâts dans un rayon de {**Secondaire.x**} m.
* Phantom's Translocuteur: À activer pour marquer un lieu pendant {**Primaire.x**} s. À activer à nouveau pour vous y téléporter après un délai de {**Secondaire./**} s.
* Scorpion's Trauma: Un projectile à tête chercheuse qui inflige {**Primaire.x**} points de dégâts, puis {**Secondaire.x**} par seconde à chaque module. Recharge immédiate si la cible est détruite pendant son activation.
* Harpy's Douille de napalm: Tire un projectile qui inflige {**Secondaire.x**} points de dégâts d'explosion à l'impact, puis {**Primaire.x**} dégâts de feu au fil du temps.
* Bulwark's Sphère absorbante: Atténue {**Secondaire.x**} % des dégâts reçus pendant {**Primaire.x**} s. Les projectiles pris dans la sphère rechargent vos munitions.
* Kumo's Armure de distorsion: Le robot se téléporte à l’emplacement choisi et confère une réduction de dégâts de {**Primaire.x**} % à lui-même et aux alliés proches. L’effet dure {**Secondaire.x**} s.
* Anansi's Toile de camouflage: Crée une toile de camouflage avec un rayon de {**Secondaire.x**} m pendant {**Primaire.x**} s qui rend les alliés qui s'y trouvent invisibles pour les ennemis à l'extérieur. Temps de recharge : {CD}.
* Fury's Recharge de distorsion: Régénère {**Secondaire.x**} % des munitions après un délai de {**Primaire./**} s.
* Pursuer's Camouflage optique: Après {**Secondaire./**} s secondes, vous devenez invisible pendant {**Primaire.x**} s. Vous pouvez utiliser les modules de mécanisme lorsque vous êtes invisible, le tir de vos armes révélera immédiatement le robot. Temps de recharge : {CD}.
* Ares's Colère: Active une barrière intégrée et des armes supplémentaires qui infligent {**Primaire.x**} points de dégâts par tir pendant {**Secondaire.x**} s.
* Griffin's Brouilleur: Empêche les ennemis de prendre vos alliés pour cibles dans un rayon de {**Primaire.x**} m et répartit les dégâts subis entre les modules. Dure {**Secondaire.x**} s. Tout robot affecté ne peut être détruit.
* Lancelot's Ordre de bataille: Les alliés situés dans un rayon de {**Primaire.x**} m subissent {**Secondaire.x**} % points de dégâts en moins. S'ils sont touchés, votre blindage se régénère.
* Ravana's Matrice gardienne: Les alliés devant vous subissent {**Secondaire.x**} % points de dégâts en moins pendant {**Primaire.x**} s. Vous êtes invincible tant qu'un allié est touché par cet effet.
* Siren's Coup de froid: Tire un projectile glaçant qui inflige {**Primaire.x**} points de dégâts aux ennemis dans son rayon d'explosion et les immobilise pendant {**Secondaire.x**} s.
* Helios's Cavalier enflammé: S'active pendant {**Secondaire.x**} s, déposant des flaques de napalm sous le robot. Les flaques ralentissent les robots ennemis de 30% et infligent {**Primaire.x**} dégâts par seconde à chaque composant de robot dans la zone touchée. Tant qu'elle est active, le robot peut se téléporter à sa position de départ à tout moment. Temps de recharge : {CD}.
* Crix's Grappin: Libère un essaim de nanobots qui s'accrochent à la cible pendant {**Primaire.x**} s, traînant le robot vers elle et infligeant {**Secondaire.x**} points de dégâts.
* Ceres's Revitaliseur: Répare {**Primaire.x**} % du blindage par seconde. S'il y a des alliés dans les {**Secondaire.x**} m, ils sont aussi réparés, et le taux de réparation est doublé. Dure 12 secondes.
* Relic Scorpion's Frappe de l'ombre: Téléporte le robot jusqu'à {**Primaire.x**} m. En réapparaissant, inflige 25000 dégâts de bouclier à tous les ennemis dans la zone. Active également un canon arrière intégré pendant {**Secondaire.x**} s, infligeant 3000 dégâts par seconde. Temps de recharge : {CD}.
* Relic Lancelot's Protection du Paladin: Confère 35% de résistance aux dégâts pendant {**Primaire.x**} s. Tant qu'elle est active, les alliés proches restaurent une armure égale à {**Secondaire.x**} % des dégâts absorbés par Lancelot. Temps de recharge : {CD}.
* Hitcher's Drone ailé: Fait apparaître un drone aux ailes plates et longues de {**Secondaire.x**} qui inflige {**Primaire.x**} points de dégâts à tous les ennemis sur son chemin.

## Mécanisme de cycle

* Étourdissement: Tire un projectile à tête chercheuse qui inflige {**Secondaire.x**} points de dégâts et aveugle la cible pendant {**Primaire.x**} s.
* Mode reconnaissance: Votre robot gagne un boost de vitesse et révèle tous les ennemis dans un rayon de {**Secondaire.x**} m pendant {**Primaire.x**} s.
* Réparation de nanites: Répare tous les modules à hauteur de {**Primaire.x**} points de blindage par cycle. Dure {**Secondaire.x**} s.
* Afflux de ravitaillement: Après une préparation de {**Primaire./**} s, restaure 1 charge et réduit le temps de recharge du Ravitaillement de {**Secondaire.x**} %. Charge requise : {ChargeDrain}.
* Réflecteur: Renvoie {**Secondaire.x**} % des dégâts subis vers l'ennemi pendant {**Primaire.x**} s.
* Munitions infinies: Après un délai de {**Secondaire./**} s, fournit des munitions illimitées pendant {**Primaire.x**} s.
* Missile chercheur: Tire un missile à tête chercheuse qui inflige {**Primaire.x**} points de dégâts à toutes les pièces de blindage dans la zone d'explosion ou {**Secondaire.x**} aux boucliers.
* Rayon gamma: Se verrouille sur l'ennemi pendant {**Secondaire.x**} s, infligeant jusqu'à {**Primaire.x**} points de dégâts par seconde. Les dégâts augmentent avec la proximité.
* Champ de mines: Déploie un champ de mines invisible pendant {**Secondaire.x**} s. Chaque mine ralentit les ennemis à l'impact et inflige {**Primaire.x**} points de dégâts.
* Parapluie: Déploie une barrière d'un rayon de {**Secondaire.x**} m qui vous protège des attaques venues du ciel. Dure {**Primaire.x**} s.
* Voile de fer: Réduit tous les dégâts subis de {**Primaire.x**} % pendant {**Secondaire.x**} s. Charge requise : {ChargeDrain}


# Primary and Secondary Parameters in Ability Descriptions (ru)

## Оборудование поддержки

* Запас топлива: Восстанавливает до {**Второстепенное.x**}ед. топлива ({**Основное.x**}ед. топлива в ед. времени).
* Сжигание топлива: Сжигает {**Второстепенное.x**}% ед. топлива цели и блокирует восстановление топлива на {**Основное.x**}сек.. Заряды: {Consum}. Перезарядка: {CD}.
* Ударная волна: Устанавливает бомбу в пределах {**Второстепенное.x**}м. Вскоре она взрывается, нанося противникам {**Основное.x**} ед. урона и отбрасывая их.
* Подавитель: Размещает магнитный излучатель, который снижает урон врагов в радиусе действия на {**Основное.x**}% в течение {**Второстепенное.x**}сек. сек.
* Быстрый ремонт: Восстанавливает наиболее повреждённому модулю {**Основное.x**}% ед. брони в секунду. Длительность {**Второстепенное.x**}сек..
* Кинетический импульс: Выпускает мину, которая прилипает к поверхности или роботу и испускает {**Основное.x**} импульсов, отталкивая противников в радиусе {**Второстепенное.x**}м в разные стороны. Перезарядка {CD}. {Consum} зарядов.
* Скачок: Через {**Второстепенное./**}сек. телепортирует вас на расстояние {**Основное.x**}м в том направлении, куда вы смотрите.
* Призрачная турель: На {**Второстепенное.x**}сек. устанавливает турель, наносящую ближайшему противнику {**Основное.x**} ед. урона за выстрел.
* Ускоряющий реактор: На {**Основное.x**}сек. повышает скорость передвижения на {**Второстепенное.x**}км/ч.
* Экстренный щит: Ежесекундно восстанавливает {**Основное.x**} ед. энергии щита в течение {**Второстепенное.x**}сек..
* Фабрикатор боеприпасов: Размещает устройство, повышающее скорость перезарядки союзников в радиусе {**Второстепенное.x**}м на {**Основное.x**}%.
* Энергостена: Устанавливает барьер шириной {**Основное.x**}м. Он останавливает врагов и поглощает до {**Второстепенное.x**} ед. урона.
* Репульсор: Наносит {**Второстепенное.x**} ед. урона противникам и отбрасывает их и вас друг от друга с силой {**Основное.x**}м/с.
* Голографическая завеса: Устанавливает завесу шириной {**Второстепенное.x**}м, на {**Основное.x**}сек. закрывающую обзор.

## Корпус

* Mesa's Гармонизатор: Восстанавливает {**Основное.x**} прочности щитов союзников в секунду и половину от этого значения для Mesa, а также уменьшает урон противников на 30%. Область действия: {**Второстепенное.x**}м. Перезарядка: {CD}.
* Tyr's Ремонтный дрон: Разворачивает лечебный дрон, который восстанавливает союзникам в радиусе {**Основное.x**}м по 2100 единиц брони в секунду в течение {**Второстепенное.x**}сек.. Перезарядка: {CD}.
* Decker's Каскадная перегрузка: Заражает вражеского робота вирусом, который наносит {**Основное.x**} урона в секунду в течение 6 сек. Если другой противник окажется в радиусе {**Второстепенное.x**}м во время действия эффекта, вирус перескочит на него. Эффект может распространиться до 8 раз.
* Bulgasari's Железный дождь: Выпускает залп по области {**Второстепенное.x**}м. Каждая ракета наносит {**Основное.x**} ед. урона всем модулям.
* Cyclops's Луч смерти: После небольшой задержки выпускает энергетический луч, который наносит {**Основное.x**} урона в секунду в течение {**Второстепенное.x**}сек.. Ваш робот не может идти, пока она активна.
* Varangian's Энергокупол: Создаёт барьер радиусом {**Второстепенное.x**}м, не дающий врагам войти в его область действия или выйти из неё. Поглощает до {**Основное.x**} ед. урона.
* Artemis's Метка охотника: Выпускает снаряд, который при попадании обнаруживает противника, увеличивая получаемый им урон по броне на 20% в течение {**Второстепенное.x**}сек.. Если цель уничтожена во время действия эффекта, все союзники, наносившие ей урон в последние 10 секунд, восстанавливают {**Основное.x**} брони в каждой зоне бронирования. Перезарядка: {CD}.
* Relic Bulgasari's Арпеджио: Выпускает залп ракет, поражающих целевую область сверху. Каждая ракета имеет радиус взрыва {**Второстепенное.x**}м и наносит {**Основное.x**} урона каждой детали робота в зоне поражения. Перезарядка: {CD}.
* Purifier's Фанатизм: В течение {**Основное.x**}сек. урон ближайших союзников увеличивается на {**Второстепенное.x**}%. Эффект усиливается, когда союзники уничтожают противников или их щиты.
* Loki's Голографическая приманка: На {**Основное.x**}сек. скрывает вашего робота и создаёт его копию. Щиты и броня приманки прочнее на {**Второстепенное.x**}%, и на неё переносятся все активные эффекты, включая эффекты от модулей оборудования.
* Heike's Сброс оболочки: Разряжает щиты импульсом радиусом 70 м, нанося {**Основное.x**} урона вражеским щитам и давая Heike 30% сопротивления урону, а союзникам в зоне — 15% на {**Второстепенное.x**}сек. секунд.
* Orochi's Эхо-залп: Атакой по прямой линии наносит {**Основное.x**} ед. урона щитам и раскрывает противников на {**Второстепенное.x**}сек..
* Fenrir's Рагнарёк: Жертвует {**Второстепенное.Cx**}% текущей брони, увеличивая урон на 25% на 10 секунд. Восстанавливает щиты и прочность в размере {**Основное.x**}% от нанесённого урона. Перезарядка: {CD}.
* Typhon's Перебой: Выпускает самонаводящиеся ракеты, которые поражают до 3 противников и вынуждают их перезарядиться. Их скорость перезарядки снижается на {**Основное.x**}% на {**Второстепенное.x**}сек..
* Garuda's Змеехват: Выстреливает модуль, который при попадании разворачивает энергетическую клетку и не позволяет вражеским роботам проходить сквозь неё. Клетка действует {**Основное.x**}сек.. Запертые внутри противники получают штраф {**Второстепенное.x**}% к скорострельности и разбросу оружия.
* Wyrm's Утилизатор: Выпускает во врага магнитный гарпун и притягивает его в течение {**Основное.x**}сек.. Пока способность активна, робот каждую секунду наносит {**Второстепенное.x**} урона каждой детали вражеского робота в зоне перед собой. Перезарядка: {CD}.
* Raven's Стальные перья: Выпускает перед вами залп ракет. Каждая из них наносит {**Основное.x**} ед. урона в области {**Второстепенное.x**}м.
* Phantom's Транслокатор: При первой активации отмечает точку на {**Основное.x**}сек.. При второй активации телепортирует к ней через {**Второстепенное./**}сек..
* Scorpion's Плеть: Выпускает самонаводящийся снаряд, наносящий {**Основное.x**} ед. урона при попадании и {**Второстепенное.x**} ед. урона в секунду всем модулям. Если цель уничтожается во время действия эффекта, перезарядка сбрасывается.
* Harpy's Напалмовый снаряд: Выпускает снаряд, наносящий {**Второстепенное.x**} ед. взрывного урона при попадании и {**Основное.x**} ед. периодического урона огнём.
* Bulwark's Поглощающая сфера: Блокирует {**Второстепенное.x**}% урона в течение {**Основное.x**}сек.. Оказавшиеся в сфере снаряды пополняют ваш боезапас.
* Kumo's Варп-броня: Робот телепортируется в выбранную точку и накладывает на себя и ближайших союзников эффект снижения урона на {**Основное.x**}%. Эффект действует {**Второстепенное.x**}сек..
* Anansi's Маскировочная сеть: На {**Основное.x**}сек. создаёт маскировочную сеть радиусом {**Второстепенное.x**}м. Все союзники в зоне действия становятся невидимыми для противников. Перезарядка: {CD}.
* Fury's Варп-перезарядка: Через {**Основное./**}сек. восполняет {**Второстепенное.x**}% боезапаса.
* Pursuer's Оптическая маскировка: Через {**Второстепенное./**}сек. вы на {**Основное.x**}сек. становитесь невидимы. Во время невидимости можно использовать модули снаряжения. Стрельба моментально делает робота видимым. Перезарядка: {CD}.
* Ares's Гнев: Активирует встроенный барьер и дополнительное оружие, наносящее {**Основное.x**} ед. урона за выстрел в течение {**Второстепенное.x**}сек..
* Griffin's Подавитель сигнала: Не позволяет противникам захватывать цели в пределах {**Основное.x**}м и разделяет получаемый урон между модулями. Длительность: {**Второстепенное.x**}сек.. Находящиеся под действием эффекта роботы не могут быть уничтожены.
* Lancelot's Боевой порядок: Союзники в области действия ({**Основное.x**}м) получают на {**Второстепенное.x**}% меньше урона. Когда в них попадает противник, ваша броня восстанавливается.
* Ravana's Защитная матрица: Союзники перед вами получают на {**Второстепенное.x**}% меньше урона в течение {**Основное.x**}сек.. Пока этот эффект действует хотя бы на одного союзника, вы неуничтожимы.
* Siren's Заморозок: Выпускает замораживающий снаряд. Он наносит противникам {**Основное.x**} ед. урона и обездвиживает их на {**Второстепенное.x**}сек..
* Helios's Огненный всадник: Активируется на {**Второстепенное.x**}сек., оставляя лужи напалма под роботом. Лужи замедляют вражеских роботов на 30% и наносят {**Основное.x**} урона в секунду каждому модулю робота в зоне поражения. Пока способность активна, робот может в любой момент телепортироваться обратно в исходную позицию. Перезарядка: {CD}.
* Крикс's Взлом: Выпускает рой наноботов, которые прикрепляются к цели на {**Основное.x**}сек., притягивая к ней робота и нанося {**Второстепенное.x**} урона.
* Ceres's Регенератор: Восстанавливает {**Основное.x**}% брони в секунду. Если союзники находятся в радиусе {**Второстепенное.x**}м, броня их роботов восстанавливается с удвоенной скоростью. Время действия: 12 секунд.
* Relic Scorpion's Удар из тени: Телепортирует робота на расстояние до {**Основное.x**}м. При появлении наносит 25000 урона щитам всех врагов в зоне поражения. Также активирует встроенное хвостовое орудие на {**Второстепенное.x**}сек., наносящее 3000 урона в секунду. Перезарядка: {CD}.
* Relic Lancelot's Оберег Паладина: Дает 35% устойчивости к урону на {**Основное.x**}сек.. Пока способность активна, союзники поблизости восстановят броню в размере {**Второстепенное.x**}% от урона, поглощенного Lancelot. Перезарядка: {CD}.
* Hitcher's Крылатый дрон: Вызывает дрона с плоскими крыльями длиной {**Второстепенное.x**}, наносящего {**Основное.x**} ед. урона всем противникам на пути.

## Специальное оборудование

* Ослепление: Выпускает самонаводящийся снаряд, наносящий {**Второстепенное.x**} ед. урона и ослепляющий цели на {**Основное.x**}сек..
* Режим разведки: Робот ускоряется и раскрывает всех противников в пределах {**Второстепенное.x**}м. Время действия: {**Основное.x**}сек..
* Наноремонт: Чинит все модули, восстанавливая {**Основное.x**}% брони в ед. времени. Длительность: {**Второстепенное.x**}сек..
* Всплеск снабжения: После {**Основное./**}сек. подготовки, восстановит 1 заряд и сократит перезарядку Оборудования поддержки на {**Второстепенное.x**}%. Затраты заряда: {ChargeDrain}.
* Отражатель: В течение {**Основное.x**}сек. отражает {**Второстепенное.x**}% получаемого урона в противника.
* Бесконечные снаряды: Через {**Второстепенное./**}сек. боезапас становится бесконечным на {**Основное.x**}сек..
* Умная ракета: Выпускает самонаводящуюся ракету, наносящую {**Основное.x**} ед. урона броне в зоне взрыва или {**Второстепенное.x**} ед. урона щитам.
* Гамма-луч: Наводится на противника и в течение {**Второстепенное.x**}сек. наносит до {**Основное.x**} ед. урона в секунду (урон зависит от расстояния).
* Минное поле: На {**Второстепенное.x**}сек. устанавливает невидимое минное поле. Каждая мина при срабатывании замедляет противников и наносит {**Основное.x**} ед. урона.
* Зонт: Создаёт барьер радиусом {**Второстепенное.x**}м, защищающий от атак сверху. Длительность: {**Основное.x**}сек..
* Железная завеса: Уменьшает весь получаемый урона на {**Основное.x**}% в течение {**Второстепенное.x**}сек.. Расход зарядов: {ChargeDrain}.


# Primary and Secondary Parameters in Ability Descriptions (zh-Hant)

## 補給裝備

* 保存燃料: 每秒補充最多 {**次要.x**}燃料單位 燃料（單位：{**主要.x**}燃料單位）。
* 燃料燒灼: 燃燒目標 {**次要.x**}% 燃料並封鎖其恢復能力，持續 {**主要.x**}秒。冷卻時間 {CD}，消耗 {Consum} 充能。
* 衝擊波爆破: 在 {**次要.x**}公尺 範圍内部署一顆炸彈。短暫延遲後，爆炸會造成 {**主要.x**} 傷害並擊退敵人。
* 抑制器: 部署一個黏性信號發射器，使範圍內敵人的傷害降低 {**主要.x**}%，持續 {**次要.x**}秒。
* 快速修復: 修復受損最嚴重的模組，每秒恢復 {**主要.x**}% 裝甲。持續 {**次要.x**}秒。
* 動能脈衝: 發射一枚可附著於表面或機甲的射彈，並發出 {**主要.x**} 道推力脈衝，將半徑 {**次要.x**}公尺 內的敵人向外推開。冷卻時間 {CD}。{Consum} 次充能。
* 閃現: 延遲 {**次要./**}秒 之後，朝你面對的方向傳送 {**主要.x**}公尺。
* 鬼影炮塔: 部署砲塔，砲塔會攻擊距離最近的敵人，每發造成 {**主要.x**} 傷害，持續 {**次要.x**}秒。
* 衝刺反應爐: 最快速度提升 {**次要.x**}公里/小時，持續 {**主要.x**}秒。
* 緊急護盾: 每秒恢復 {**主要.x**} 護盾，持續 {**次要.x**}秒。
* 彈藥製造商: 部署裝置，該裝置可以讓 {**次要.x**}公尺 範圍內的盟友填裝速度縮短 {**主要.x**}%。
* 能量障壁: 部署寬度為 {**主要.x**}公尺 的屏障，能夠阻撓敵人並吸收最多 {**次要.x**} 傷害。
* 斥力衝擊: 對敵人造成 {**次要.x**} 傷害，並以 {**主要.x**}m/s 力量將你和敵人推開。
* 全息場: 部署寬度為 {**次要.x**}公尺 的墻面，阻擋視線，持續 {**主要.x**}秒。

## 機體

* Mesa's 協調器: 每秒為 {**次要.x**}公尺範圍內友軍恢復 {**主要.x**} 護盾，Mesa 本身會恢復前述數值的一半。還會減少範圍內敵軍傷害輸出 30%。 冷卻: {CD}.
* Tyr's 維修子機: 部署一架治療無人機，在 {**次要.x**}秒 內為 {**主要.x**}公尺 範圍內的友軍每秒恢復 2100 裝甲。冷卻時間：{CD}。
* Decker's 鏈式超載: 使敵方機甲感染病毒，在 6 秒內每秒造成 {**主要.x**} 傷害。效果期間，若有其他敵人進入 {**次要.x**}公尺 範圍，病毒就會轉移至其身上。最多可連鎖 8 次。
* Bulgasari's 鋼鐵火雨: 轟炸特定範圍，在 {**次要.x**}公尺 爆破半徑內每一枚火箭對所有模組造成 {**主要.x**} 傷害。
* Cyclops's 死亡射線: 短暫延遲後，釋放能量束，在 {**次要.x**}秒 內每秒造成 {**主要.x**} 傷害。激活期間，你的機器人無法行走。
* Varangian's 能量穹頂: 部署範圍 {**次要.x**}公尺 的屏障。該屏障能吸收最多 {**主要.x**} 來自敵人的傷害。
* Artemis's 獵人印記: 發射一枚砲彈，擊中時顯露敵人，並在 {**次要.x**}秒 內使其受到的裝甲傷害增加 20%。如果目標在受影響期間被摧毀，所有在過去 10 秒內對其造成傷害的盟友，其每個裝甲區域都將恢復 {**主要.x**} 裝甲。冷卻時間：{CD}。
* Relic Bulgasari's 琶音: 向目標區域發動從天而降的火箭彈齊射。每枚火箭彈的爆炸半徑為 {**次要.x**}公尺，並對爆炸區域內的每個機器人部件造成 {**主要.x**} 點傷害。冷卻時間：{CD}。
* Purifier's 堅盾狂徒: 附近友軍造成的傷害提高 {**次要.x**}%，持續 {**主要.x**}秒。當隊友擊殺或摧毀護盾時，強化規模提高。
* Loki's 投影誘餌: 匿蹤並投射出你的機器人複製體，持續 {**主要.x**}秒。誘餌的裝甲和護盾提高 {**次要.x**}%，且包含裝備模組上的所有啟用效果也將一同複製。
* Heike's 護甲蛻殼: 釋放護盾脈衝，對70公尺範圍內敵方護盾造成 {**主要.x**} 傷害，並在 {**次要.x**}秒 秒內為 Heike 提供30%傷害抗性，同時為範圍內盟友提供15%傷害抗性。
* Orochi's 回音火網: 在直線上造成 {**主要.x**} 護盾傷害並暴露敵人，持續 {**次要.x**}秒。
* Fenrir's 諸神殞落: 犧牲當前護甲的 {**次要.Cx**}%，使傷害提升 25%，持續 10 秒。依造成傷害的 {**主要.x**}% 恢復護盾與結構。冷卻時間：{CD}。
* Typhon's 強制填裝: 發射追蹤飛彈，這些飛彈會迫使最多 3 名敵人填裝，同時減緩其填裝速度 {**主要.x**}%，持續 {**次要.x**}秒。
* Garuda's 捕蛇者: 發射一個模組，命中後展開能量牢籠，阻止敵方機器人通過。牢籠持續 {**主要.x**}秒。被困其中的敵人射速和武器擴散降低 {**次要.x**}%。
* Wyrm's 回收者: 向目標發射一枚磁力魚槍，並將其拉近，持續 {**主要.x**}秒。啟用期間，機器人對前方區域內的每個敵方機器人部件每秒造成 {**次要.x**} 點傷害。冷卻時間：{CD}。
* Raven's 鐵翼彈雨: 朝正前方發射一大批飛彈，每一枚飛彈造成 {**主要.x**} 傷害，爆破半徑為 {**次要.x**}公尺。
* Phantom's 定位傳送: 啟動時會標記傳送點，持續 {**主要.x**}秒。再次啟動就能在延遲 {**次要./**}秒 後回傳至該傳送點。
* Scorpion's 鞭撻: 命中時造成 {**主要.x**} 點傷害的追蹤投射物，接著每秒對所有模組造成 {**次要.x**} 點傷害。若目標在此技能生效時死亡，此技能會立即再次充能。
* Harpy's 火海試煉: 發射投射物，命中後造成 {**次要.x**} 傷害，然後持續造成 {**主要.x**} 燃燒傷害。
* Bulwark's 球體吸收: 阻擋來襲傷害的 {**次要.x**}%，持續 {**主要.x**}秒。進入球形範圍內的投射物會填裝你的彈藥。
* Kumo's 曲速裝甲: 機器人傳送至指定位置，並為自身及附近所有盟友施加 {**主要.x**}% 的傷害減免效果。該增益持續 {**次要.x**}秒。
* Anansi's 偽裝網: 部署範圍 {**次要.x**}公尺 的偽裝網，持續 {**主要.x**}秒，使網中的友軍隱形，無法被偽裝網範圍外的敵人看見。冷卻時間 {CD}。
* Fury's 蟲洞填裝: 延遲 {**主要./**}秒 之後，補充 {**次要.x**}% 彈藥。
* Pursuer's 光學迷彩: 在延遲 {**次要./**}秒 秒後隱形，持續 {**主要.x**}秒。隱形時可使用裝備模組，但使用武器開火時機器人會立即顯形。冷卻時間：{CD}。
* Ares's 怒不可遏: 啟用內建屏障和額外武器，每一發造成 {**主要.x**} 傷害，持續 {**次要.x**}秒。
* Griffin's 脈衝干擾: 防止敵人鎖定 {**主要.x**}公尺 範圍內的友軍，且會將來襲的傷害分攤至各個模組之上。持續 {**次要.x**}秒。受到影響的所有機器人無法被摧毀。
* Lancelot's 對戰命令: {**主要.x**}公尺範圍內的盟友承受的傷害減少 {**次要.x**}%。盟友只要遭到擊中，你的裝甲就會恢復。
* Ravana's 守護矩陣: 你前方的友軍承受的傷害減少 {**次要.x**}%，持續 {**主要.x**}秒。在任一友軍受此影響的期間你無法被摧毀。
* Siren's 至寒重擊: 發射急凍投射物，在爆破半徑內對敵人造成 {**主要.x**} 傷害並將其凍結，持續 {**次要.x**}秒。
* Helios's 烈焰騎手: 啟用持續{**次要.x**}秒，在機器人下方留下凝固汽油池。水池使敵方機器人減速30%，並對受影響區域內的每個機器人部件每秒造成{**主要.x**}傷害。啟用時，機器人可以隨時傳送回初始位置。冷卻時間：{CD}。
* Crix's 擒拿: 釋放一群奈米機器人，在 {**主要.x**}秒 內鎖定目標，將機器人拖向目標並造成 {**次要.x**} 傷害。
* Ceres's 再生矩陣: 每秒修復 {**主要.x**}% 裝甲。如果 {**次要.x**}公尺 範圍內有友軍，修復率則會加倍，並同時修復友軍。持續 12 秒。
* Relic Scorpion's 暗影突襲: 將機器人傳送至多{**主要.x**}公尺。重新出現時，對區域內的所有敵人造成25000點護盾傷害。此外還會激活內置尾砲，持續{**次要.x**}秒，每秒造成3000點傷害。冷卻時間：{CD}。
* Relic Lancelot's 聖騎士之護: 提供35%的傷害抗性，持續{**主要.x**}秒。啟用時，附近的盟友可恢復等同於Lancelot吸收傷害{**次要.x**}%的護甲。冷卻時間：{CD}
* 搭載器's 翼型無人機: 召喚一架翼展長達 {**次要.x**} 的無人機，對沿途所有敵人造成 {**主要.x**} 傷害。

## 循環裝備

* 震撼彈: 發射追蹤裝置，該裝置會造成 {**次要.x**} 傷害並使目標失去視野，持續 {**主要.x**}秒。
* 情報員模式: 機器人獲得加速並暴露 {**次要.x**}公尺 範圍內的所有敵人，持續 {**主要.x**}秒。
* 奈米修復: 每秒恢復所有模組 {**主要.x**}% 裝甲。持續 {**次要.x**}秒。
* 補給暴增: 蓄力 {**主要./**}秒 後，恢復 1 個充能，並將補給裝置的冷卻時間縮短 {**次要.x**}%。費用：{ChargeDrain}。
* 反射器: 將來襲傷害的 {**次要.x**}% 反彈回給敵人，持續 {**主要.x**}秒。
* 無限彈藥: 延遲 {**次要./**}秒 之後，獲得無限彈藥，持續 {**主要.x**}秒。
* 追蹤導彈: 發射追蹤飛彈，該飛彈會對爆炸範圍內的所有裝甲部分造成 {**主要.x**} 傷害，或對護盾造成 {**次要.x**} 傷害。
* 伽瑪束: 鎖定敵人，持續 {**次要.x**}秒，每秒造成最多 {**主要.x**} 傷害。距離目標越近，傷害越高。
* 雷區: 建立一片隱形雷區，持續 {**次要.x**}秒。每顆地雷都會讓踩中的敵人緩速並造成 {**主要.x**} 傷害。
* 保護傘: 設下範圍 {**次要.x**}公尺 的屏障，抵禦空襲。持續 {**主要.x**}秒。
* 鋼鐵紗幕: 減少 {**主要.x**}% 受到的所有傷害，持續 {**次要.x**}秒。冷卻時間：{ChargeDrain}。


# Primary and Secondary Parameters in Ability Descriptions (en)

## Supply Gear

* Fuel Reserve: Refills up to {**Secondary.x**}fuel units fuel at {**Primary.x**}fuel units units per tick.
* Fuel Burn: Burns {**Secondary.x**}% fuel from the target and blocks its regeneration for {**Primary.x**}s. Cooldown {CD}, {Consum} charges.
* Blast Wave: Plants a bomb within {**Secondary.x**}m. After a short delay, explosion inflicts {**Primary.x**} damage and knocks back enemies.
* Suppressor: Deploys a sticky emitter, which reduces enemies' damage in area by {**Primary.x**}% for {**Secondary.x**}s.
* Quick Repair: Repairs your most damaged module for {**Primary.x**}% of armor per second. Lasts {**Secondary.x**}s.
* Kinetic Pulse: Fires a projectile that sticks to a surface or robot and emits {**Primary.x**} pulses, pushing enemies at a distance of {**Secondary.x**}m in different directions. Cooldown {CD}. {Consum} charges.
* Blink: After a {**Secondary./**}s delay, teleport over {**Primary.x**}m in a direction you're facing.
* Ghost Turret: Deploys a turret that attacks the closest enemy for {**Primary.x**} damage per shot over {**Secondary.x**}s.
* Sprint Reactor: Boosts you maximum speed by {**Secondary.x**}km/h for {**Primary.x**}s.
* Emergency Shield: Restores {**Primary.x**} shield per second for {**Secondary.x**}s.
* Ammo Fabricator: Deploys a device that reduces the reload time of allies within {**Secondary.x**}m by {**Primary.x**}%.
* Energy Wall: Deploys a {**Primary.x**}m wide barrier that stops enemies and absorbs up to {**Secondary.x**} damage.
* Repulsor: Deals {**Secondary.x**} damage to enemies, and pushes them and you away from each other with {**Primary.x**}m/s force.
* Holofield: Deploys a {**Secondary.x**}m wide wall that blocks line of sight for {**Primary.x**}s.

## Torso

* Mesa's Harmonizer: Restores {**Primary.x**} shield per second for allies within {**Secondary.x**}m and half this value for Mesa itself. Also reduces damage output for enemies in range by 30%. Cooldown: {CD}.
* Tyr's Repair Drone: Deploys a healing drone, restoring 2100 armor per second to allies within {**Primary.x**}m for {**Secondary.x**}s. Cooldown {CD}.
* Decker's Chain Overload: Infects an enemy robot with a virus that deals {**Primary.x**} damage per second for 6 seconds. If another enemy comes within {**Secondary.x**}m during the effect, the virus jumps to them. Can chain up to 8 times.
* Bulgasari's Iron Rain: Bombards an area, dealing {**Primary.x**} damage per rocket to all modules within a {**Secondary.x**}m blast radius.
* Cyclops's Death Ray: After a short delay, unleashes an energy beam that deals {**Primary.x**} damage per second for {**Secondary.x**}s. Your robot can't walk while it's active.
* Varangian's Energy Dome: Deploys a barrier with {**Secondary.x**}m radius that absorbs up to {**Primary.x**} damage from enemy attacks.
* Artemis's Hunter's Mark: Fires a projectile that reveals an enemy on hit, increasing armor damage taken by 20% for {**Secondary.x**}s. If the target is destroyed while affected, all allies who damaged it within the last 10 seconds restore {**Primary.x**} armor to each armor zone. Cooldown: {CD}.
* Relic Bulgasari's Arpeggio: Launches a salvo of rockets that strike the targeted area from above. Each rocket has a blast radius of {**Secondary.x**}m and deals {**Primary.x**} damage to each robot component within the blast area. Cooldown: {CD}.
* Purifier's Zealot: Nearby allies deal {**Secondary.x**}% more damage for {**Primary.x**}s. Effect increases whenever they score kills or destroy shields.
* Loki's Holographic Decoy: Makes you invisible and projects a copy of your robot for {**Primary.x**}s. The decoy has {**Secondary.x**}% more armor and shields, and all active effects including from gear modules are transferred to it.
* Heike's Ecdysis: Discharges its shields in a 70 m pulse, dealing {**Primary.x**} to enemy shields and granting 30% damage resistance to Heike and 15% to allies within the area for {**Secondary.x**}s seconds.
* Orochi's Echo Barrage: Deals {**Primary.x**} shield damage in a straight line, revealing enemies for {**Secondary.x**}s.
* Fenrir's Ragnarok: Sacrifices {**Secondary.Cx**}% of current armor to increase damage by 25% for 10 seconds. Restores shield and structure for {**Primary.x**}% of damage dealt. Cooldown {CD}.
* Typhon's Blackout: Fires homing missiles that force up to 3 enemies to reload, while also slowing their reloads by {**Primary.x**}% for {**Secondary.x**}s.
* Garuda's Snake Catcher: Fires a module that deploys an energy cage on impact, preventing enemy robots from passing through it. The cage lasts for {**Primary.x**}s. Enemies trapped inside suffer a {**Secondary.x**}% penalty to rate of fire and weapon spread.
* Wyrm's Reclaimer: Fires a magnetic harpoon at a target and pulls it in for {**Primary.x**}s. While active, the robot deals {**Secondary.x**} damage per second to each enemy robot component in the area in front of it. Cooldown: {CD}.
* Raven's Steel Feathers: Fires a barrage of missiles dead ahead, each dealing {**Primary.x**} damage with a {**Secondary.x**}m blast radius.
* Phantom's Translocator: Activate to mark a spot for {**Primary.x**}s. Activate again to warp back to that spot after a {**Secondary./**}s delay.
* Scorpion's Whiplash: A homing projectile deals {**Primary.x**} damage on hit and then {**Secondary.x**} per second to all modules. Recharges if target is destroyed while active.
* Harpy's Napalm Shell: Fires a projectile that deals {**Secondary.x**} blast damage on impact, then {**Primary.x**} fire damage over time.
* Bulwark's Absorber Sphere: Mitigates {**Secondary.x**}% of incoming damage for {**Primary.x**}s. Projectiles caught in the sphere refill your ammo.
* Kumo's WarpArmor: The robot teleports to a selected spot and casts a {**Primary.x**}% damage reduction on itself and all allies nearby. The buff lasts for {**Secondary.x**}s
* Anansi's Camouflage Web: Creates a Camouflage Web with a radius of {**Secondary.x**}m for {**Primary.x**}s, making allies inside it invisible to enemies outside. Cooldown {CD}.
* Fury's Warp Reload: Refills {**Secondary.x**}% of ammo after a {**Primary./**}s delay.
* Pursuer's Optical Camo: After a {**Secondary./**}s second delay, you become invisible for {**Primary.x**}s. Gear modules can be used while invisible, but firing weapons reveals the robot immediately. Cooldown: {CD}.
* Ares's Wrath: Activates a built-in barrier and extra weapons that deal {**Primary.x**} damage per shot for {**Secondary.x**}s.
* Griffin's Scrambler: Prevents enemies from locking on to allies within {**Primary.x**}m and splits all incoming damage between modules. Lasts {**Secondary.x**}s. Any robot affected cannot be destroyed.
* Lancelot's Battle Order: Allies within {**Primary.x**}m take {**Secondary.x**}% less damage. Whenever they get hit, you regenerate armor.
* Ravana's Guardian Matrix: Allies in front of you take {**Secondary.x**}% less damage for {**Primary.x**}s. You cannot be destroyed while any ally is affected by this.
* Siren's Cold Snap: Fires a freezing projectile that deals {**Primary.x**} damage to enemies within a blast radius and immobilizes them for {**Secondary.x**}s.
* Helios's Flaming Rider: Activates for {**Secondary.x**}s, dropping pools of napalm beneath the robot. Pools slow enemy robots by 30% and deal {**Primary.x**} damage per second to each robot component within the affected area. While active, the robot can teleport back to its starting position at any time. Cooldown: {CD}.
* Crix's Grappling: Unleashes a swarm of nanobots that latch onto the target over {**Primary.x**}s, dragging the robot toward it and dealing {**Secondary.x**} damage.
* Ceres's Rejuvenator: Repairs {**Primary.x**}% of armor per second. If any allies are within {**Secondary.x**}m, it repairs them as well and doubles the repair rate. Lasts 12 seconds.
* Relic Scorpion's Shadow Strike: Teleports the robot up to {**Primary.x**}m. Upon reappearing, deals 25000 shield damage to all enemies in the area. Also activates a built-in tail cannon for {**Secondary.x**}s, dealing 3000 damage per second. Cooldown: {CD}.
* Relic Lancelot's Paladin's Ward: Grants 35% damage resistance for {**Primary.x**}s. While active, nearby allies restore armor equal to {**Secondary.x**}% of the damage absorbed by Lancelot.  Cooldown: {CD}.
* Hitcher's Winged Drone: Spawns a drone with flat {**Secondary.x**}-long wings that deals {**Primary.x**} damage to all enemies in its path.

## Cycle Gear

* Flashbang: Fires a homing projectile that deals {**Secondary.x**} damage and blinds target for {**Primary.x**}s.
* Recon Mode: Robot gains a speed boost and reveals all enemies in {**Secondary.x**}m range for {**Primary.x**}s.
* Nanite Repair: Repairs all modules for {**Primary.x**}% armor per tick. Lasts {**Secondary.x**}s.
* Supply Surge: After a {**Primary./**}s preparation, restores 1 charge and reduces the cooldown of Supply Gear by {**Secondary.x**}%. Charge needed: {ChargeDrain}.
* Reflector: Returns {**Secondary.x**}% of incoming damage back at the enemy for {**Primary.x**}s.
* Infinite Ammo: After a {**Secondary./**}s delay, grants unlimited ammo for {**Primary.x**}s.
* Homing Missile: Fires a homing missile that deals {**Primary.x**} damage to all armor segments in the blast area or {**Secondary.x**} to shields.
* Gamma Beam: Locks onto the enemy for {**Secondary.x**}s, dealing up to {**Primary.x**} damage per second. Damage scales with proximity.
* Minefield: Deploys an invisible minefield lasting {**Secondary.x**}s. Each mine slows enemies on impact and deals {**Primary.x**} damage.
* Umbrella: Deploys a barrier with a {**Secondary.x**}m radius that protects against attacks from above. Last for {**Primary.x**}s.
* Iron Veil: Reduces all damage taken by {**Primary.x**}% for {**Secondary.x**}s. Charge needed: {ChargeDrain}.