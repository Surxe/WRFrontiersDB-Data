...
class ContentTimelineGenerator:
    def __init__(self, summaries_dir='summaries', lang_code='en'):
        if lang_code not in ['en', 'id']:
            raise ValueError("lang_code must be either 'en' or 'id'")
        self.summaries_dir = summaries_dir
        self.patch_dir = os.path.join(summaries_dir, 'patch')
        self.timeline_dir = os.path.join(summaries_dir, 'timeline')
    ...
    def _load_summaries(self) -> SummaryEntry:
        """
        From summaries/patch/<from_to_version>/<lang_code>.md,
        Save each line that starts with "* Added" into 
        a SummaryEntry: {from_to_version: [lines]}
        """
        summaries = {}
        for from_to_version in os.listdir(self.patch_dir): #from_to_version e.g. 2025-03-10_to_2025-03-13
            subdir_path = os.path.join(self.patch_dir, from_to_version)
            if not os.path.isdir(subdir_path):
                continue # e.g. summaries/.gitkeep
            file_to_read = os.path.join(subdir_path, f'{self.lang_code}.md')
...
if __name__ == "__main__":
    ContentTimelineGenerator(lang_code='en')
    ContentTimelineGenerator(lang_code='id')